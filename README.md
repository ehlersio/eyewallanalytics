...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-eight A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `App.jsx`, `InfoTip.jsx`, `SeasonTypeToggle.jsx`, `SeasonChipRow.jsx`, `GameChipsRow.jsx`, `TeamOpponentPicker.jsx`, `SeasonOverlayChart.jsx`, `LiveEventRink.jsx`, `PlayerComparisonEntry.jsx`, `SeasonComparisonPicker.jsx`, and `TeamPicker.jsx` are all fully translated. This sub-PR covers `PlayerComparisonPopup.jsx` (the "vs Player" two-player comparison popup opened from both player popups): the header's "vs" divider and close button, the 4 skater / 3 goalie tab labels (reusing existing keys from `teamView.splits.*`/`team.sectionSpecialTeams`/`teamView.tabs.advanced` wherever byte-identical, minting new `playerComparisonPopup.tabs.*` keys only for "Possession"/"Physical"/"Performance", which had no match anywhere), the goalie-vs-skater hard-block message, the position-mismatch soft-warning badge, the PWHL "possession metrics not available yet" explainer, and both percentile-bar column components — all of which reuse `PlayerPopup.jsx`'s existing `playerPopup.analytics.*`/`shotMapView.goalieCard.gsax` keys byte-for-byte, since these are the exact same stat categories already translated there. Also fixed two latent `t`-shadowing hazards (`.map(t =>` / `.find(t =>` over the tabs array, renamed to `tabDef`) before they could cause a real bug. Found and fixed a real, already-live bug via testing this exact file's position-mismatch badge: `nhlPlayerStats.js`'s and `pwhlPlayerStats.js`'s shared `posLabel()` utility returned hardcoded English position names ("Defence", "Left Wing", etc.) regardless of locale — live on the position chip in the header of the already-shipped, already-"fully translated" `PlayerPopup.jsx` and `PWHLPlayerPopup.jsx` this whole time. Fixed both utilities via the established `i18n` singleton import pattern (same non-React-context approach as `advancedStats.js`/`carContracts.js`), with two separate key sets (`posLabel.nhl.*` masculine, `posLabel.pwhl.*` feminine) rather than one shared set — since each file's `posLabel` is exclusively called for its own sport, matching this app's existing convention of natural per-sport gendering rather than forcing gender-neutral phrasing (which is reserved for genuinely cross-sport shared components). Live-verified in French: the position chip now reads "Défenseur" on an NHL defenseman and "Défenseure" on a PWHL defenseman (previously "Defence" in both); the comparison popup's "vs" divider, all 4 tab labels ("Marqueurs"/"Possession"/"Physique"/"Unités spéciales"), and the position-mismatch badge ("Écart de position : Centre contre Defence…", correctly interpolated). One console error surfaced during testing ("i18n is not defined") was chased down and confirmed to be a stale artifact in the browser tool's own message buffer — it persisted unchanged through a full dev-server restart, hard navigation, and `console.clear()`, while the live-fetched served source and the actual rendered DOM output were both confirmed correct throughout. All 24 keys used in `PlayerComparisonPopup.jsx` (plus the new `posLabel.*` keys) audited to resolve in both locale files; ESLint and the full vitest suite (185/185, confirming the new `i18n` imports in two more plain-JS utils don't trip the Node/`navigator` guard) clean; `player-comparison.cy.js` (16/16) and `pwhl-players.cy.js` (89/89) green, `players.cy.js` (72/72 on its own — one failure seen only when run back-to-back with two other heavy specs, confirmed as the documented local dev-server-under-load flake via a clean solo rerun). Remaining long tail: `TeamComparisonPopup.jsx` and `DraftTab.jsx` (large, `DraftTab.jsx` being the biggest single file left at ~586 lines / 30+ strings); `DevReplayView.jsx`/`PWHLDevReplayView.jsx`/`DevDraftView.jsx` are dev-only routes gated behind `import.meta.env.DEV` and never ship to real users, so they're deliberately out of scope. AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) still needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
