...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-two A2 sub-PRs done, `PlayerPopup.jsx` now fully translated) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, and now `PlayerPopup.jsx` are all fully translated. This sub-PR (3 of 3 for this file) covers the main popup body: season/career section headers, the 6-field bio grid, the header's weight/shoots-catches/birth-age composition, the rankings banner (division/conference/league + GAA sub-banner), the full Contract & Value panel (basic labels, value badges, P/60 chip, ELC chip, goalie GSAX badge and all their InfoTip tooltips), the 5-tab toggle, the Stats tab's empty state, and the entire Compare tab (season-picker empty state, per-game trend chart label/select/season-total suffix, loading state, no-data-for-season message). Live testing surfaced a real bug in a file this sub-PR doesn't otherwise touch: `carContracts.js`'s `valueLabel()`/`goalieValueLabel()` returned hardcoded English tier strings ("Below average", "Great value", etc.) even in French — the value badge next to a player's contract kept showing English regardless of locale. Fixed by importing the `i18n` singleton directly into that plain-JS util (same non-React-context pattern used for `advancedStats.js`'s `computeGSAx()` two sub-PRs ago) and routing all six tier labels through new `playerPopup.contract.tier*` keys; confirmed the existing `navigator`-guard fix from that earlier regression already covered this new import path (vitest stayed green). Verified live in French on Sebastian Aho (bio grid, contract labels, rankings, tabs, Compare tab's season-total suffix and per-game trend chart) and Pyotr Kochetkov (goalie contract value badge: "Sous la moyenne" / "BEA -0.4/M$" — confirming both the tier-label fix and the GSAX interpolation restructuring needed to compose the +/− sign into the value before interpolating). A skater value badge/P/60 chip never actually rendered against real dev-environment data for any roster player checked (Aho, Slavin, Svechnikov) — pre-existing data-availability behavior unrelated to this change, not a regression, and the identical code path was confirmed correct via the goalie badge. All 138 `playerPopup.*` keys used in this file audited to resolve in both locale files; ESLint and the full vitest suite (185/185) clean. The rest of the app's ~24 remaining view/component files still need string extraction (long tail: `PWHLPlayerPopup.jsx` and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
