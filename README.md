...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-five A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, `AboutPopup.jsx`, and `PlayerSearch.jsx` are all fully translated. This sub-PR is a batch of 6 small shared files that had never been touched: `App.jsx` (skip-link, `<main>` aria-label, route-Suspense loading fallback — reused `common.loading` byte-for-byte), `InfoTip.jsx` (the "ⓘ" tooltip trigger's aria-label, used app-wide — new `infoTip` namespace), `SeasonTypeToggle.jsx` (Regular/Playoffs segmented toggle, shared by both shot maps — new `seasonTypeToggle` namespace since NHL's existing "Regular Season"/"Playoffs" keys are the full phrase, not this component's compact single-word buttons), `SeasonChipRow.jsx` ("More seasons" overflow aria-label), `GameChipsRow.jsx` (the "All {{count}}" chip; its "🔴 LIVE" text reused `shotMapView.scoreBar.liveIndicator` byte-for-byte), and `TeamOpponentPicker.jsx` (the opponent-team `<select>` used inside the not-yet-translated `TeamComparisonPopup.jsx` — translating the leaf control ahead of its parent popup, same precedent as `PercentileBar.jsx` before `PlayerPopup.jsx`). Fixed a latent `t`-shadowing hazard in `TeamOpponentPicker.jsx` (its `.filter`/`.map` callbacks used `t` as the loop variable) by renaming to `team` before wiring in `useTranslation()`. Live-verified in French: `SeasonTypeToggle` ("Régulière"/"Séries" on the shot map), `SeasonChipRow` ("Plus de saisons"), `GameChipsRow` ("Tous 30" on the PWHL shot map), `InfoTip` (62 tooltip triggers all showing "Plus d'informations"), and `App.jsx`'s skip-link/main-content aria-labels. All 12 keys used across the 6 files audited to resolve in both locale files; ESLint and the full vitest suite (185/185) clean; `shot-map.cy.js` (48/48), `pwhl-shot-map.cy.js` (48/48), and `players.cy.js` (63/63) all green. Remaining long tail per a fresh full-codebase audit this session: `SeasonOverlayChart.jsx`, `LiveEventRink.jsx`, `PlayerComparisonEntry.jsx`, and `SeasonComparisonPicker.jsx` (medium, shared across sports); `TeamPicker.jsx`, `PlayerComparisonPopup.jsx`, `TeamComparisonPopup.jsx`, and `DraftTab.jsx` (large, `DraftTab.jsx` being the biggest single file left at ~586 lines / 30+ strings); `DevReplayView.jsx`/`PWHLDevReplayView.jsx`/`DevDraftView.jsx` are dev-only routes gated behind `import.meta.env.DEV` and never ship to real users, so they're deliberately out of scope. AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) still needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
