...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twelve A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, and `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx` are all fully translated. This sub-PR added the post-game box-score popups: `GameStatsPopup.jsx`+`GameStatsComponents.jsx` (NHL) and `PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx` (PWHL) — score header, period-by-period scoring, three stars, team-stat comparison bars, Corsi/Fenwick/PDO/Puck Luck (NHL only — no PWHL equivalent), skater/goalie tables, and the AI Game Summary card's share text. Stat-code abbreviations (SA/SV/SV%/TOI/GSAx on goalie rows, PP/SH/EN strength chips) were deliberately left untranslated, consistent with prior sub-PRs' treatment of bare stat codes. Also fixed `PWHLGameStatsPopup.jsx`'s hardcoded English `MONTH_NAMES` array (same bug class fixed four times already) and `nhlApi.js`'s shared `formatGameDate()` (hardcoded `'en-US'` `toLocaleDateString` call, used app-wide) by routing both through `formatters.js`'s `formatDate()` Intl wrapper. Verified live in French end-to-end for the PWHL popup (real completed game, all sections including the date and CTA); the NHL popup was verified via Cypress only (`pwhl-schedule.cy.js`'s "Game stats popup" suite + `schedule.cy.js`, both green) since the 2026-27 NHL season hadn't started any games yet at verification time (0 completed games, no season picker on NHL's `ScheduleView.jsx` to reach historical ones) — a live-data gap, not a code-coverage gap. The rest of the app's ~34 remaining view/component files still need string extraction (long tail: TriviaFeed, StatTileGrid, `ScoutingTab.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
