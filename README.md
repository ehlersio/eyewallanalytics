...
### Pending
- [ ] French/English localization (Phase A0 + A1 + nineteen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, and `ScoutingTab.jsx` are all fully translated. This sub-PR added `PeriodSummary.jsx` (the NHL period/game AI-summary popup and its own 1080×1080 share canvas — stat grid, hat trick chips, goal carousel, collapsible penalties, period breakdown, three stars). `ShareButtons.jsx`'s 3rd and final untranslated consumer picked up French here — every consumer of that shared component is now translated. Caught and fixed a real bug before it shipped, found by an existing Cypress assertion rather than manual review: initially reused `gameStatsPopup.teamStats.faceoffWinPct` ("Faceoff Win %", with a space) for this file's own `'Faceoff Win%'` (no space) — `period-summary.cy.js`'s "stat grid uses team abbr in labels" test failed on the exact string, correctly catching that these two strings, despite looking like an obvious dedup candidate, weren't byte-identical after all. Minted a separate `periodSummary.stats.faceoffWinPct` key instead, matching this migration's established discipline more strictly than the reuse instinct suggested — a concrete case where "looks like a duplicate" and "is byte-identical" turned out to differ by exactly one space. Found this file's own dev-only verification path — `/?mockGame=<gameId>`, the same query param `period-summary.cy.js` uses — and used it to open the real popup and verify all sections in French live (stat labels, AI narrative, goals/penalties/period-breakdown/three-stars sections, Save Image/Post to X buttons), rather than settling for Cypress-only verification as in recent prior sub-PRs. `period-summary.cy.js` 29/29 green after the fix. The rest of the app's ~25 remaining view/component files still need string extraction (long tail: `PWHLPeriodSummary.jsx` — this file's PWHL sibling — `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
