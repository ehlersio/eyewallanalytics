...
│   │   ├── PWHLPlayerPopup.jsx         # PWHL player popup (Stats, Heat Map, Scout, Compare — season-over-season, up to 4 seasons, one fetch per season; Compare tab adds a per-game trend chart for chart-ready metrics (6/11 skater, 2/9 goalie — box-score-backed via /pwhl/player-game-log) alongside the existing tile rows, Session 70; header also renders a "vs Player" entry point, Session 91 — a separate affordance from the "🆚 Compare" tab, which means something different (own-season history, not another player)). Skaters' header still uses PWHLHeaderPanel's 2x2 percentile-tile grid (only 4 pwhl_percentiles.py categories, not radar-worthy); goalies get a real 6-axis radar chart instead (PWHLGoalieHeaderPanel, added 2026-08 alongside eyewall-pipeline's pwhl_goalie_percentiles.py — same richness as NHL's own goalie radar, so no reason to settle for tiles there)
