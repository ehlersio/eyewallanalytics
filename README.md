...
| Player Search | Fuse.js — client-side fuzzy match against the Worker's flat NHL+PWHL+AHL+ECHL player index (`GET /players-search-index`, ~4,900 players — small enough to ship once per session rather than query per keystroke) |
...
│   │   ├── GuestGameView.jsx           # /game/:gameId?as=BOS — one NHL game watched from a team that isn't the favorite: ShotMapView inside a GameTeamProvider, pinned to that game (no season/game history). Opened from a live NHL game's team row on the Scoreboard. A "Viewing as BOS · Back to CAR" bar sits on top, and the app wears the guest team's colors while it's open. The guest view keeps its own clock (not the Topbar's), doesn't list its summaries in the bell, and offers no Live Activity. Invalid links and the favorite's own team redirect to /
...
Full detail (the NHL/PWHL resolution logic itself, the `feed=statviewfeed` vs `modulekit` bug, why PWHL resolution prefers regular seasons over more-recent playoffs, and `eyewall-poller`'s schedule-lookahead resolution added 2026-09 so `TEAM_CONFIG.season` can correctly anticipate a new season before any of its games exist) lives in `eyewall-poller`'s own README — this section only covers the frontend-facing side.

**`nhlApi.js`'s `getTeamStats()` — real prior-season fallback, not null (added 2026-09):** the schedule-lookahead resolution above means `TEAM_CONFIG.season` now routinely gets ahead of `/standings/now`'s own data for the first few weeks of a season — expected, not a bug. `_getTeamStats()` used to return `null` on that mismatch (reasoning: don't silently mislabel a stale full season as current). That broke more than it protected: the whole Scouting tab stat-comparison section disappeared, and — less obviously — `ScoutingShareCanvas` never got the props it needs to attach its own `canvasRef`, so the Share button silently no-opped too (`getNode()` always resolved `null`). Now returns the real, current *prior*-season row instead, tagged `isPriorSeason: true` + `statsSeasonId`; `ScoutingTab.jsx`'s comparison section shows an explicit "Showing `<season>` stats — this season hasn't started yet" note rather than presenting it as current. Same "carry forward real data, label it" pattern as `players-search-index`'s `teamStale`/`teamSeason` fields above, and as `eyewall-pipeline`'s `line_combinations.py` prior-season blend for the Scouting tab's lines. The Team page's regular-season stats all follow `statsSeasonId` the same way (added 2026-09): Overview's rank badges and Blks/GP, and every Advanced and Splits figure (Corsi/Fenwick, PP/PK, blocks, home/away splits, xGF% trend) — `getTeamCorsi`/`getTeamRealtime`/`getTeamPowerplay`/`getTeamPenaltyKill`/`getTeamHomeSplit` take an optional `season` for it. Advanced and Splits label it "Showing `<season>` Regular Season stats"; before this they read the empty current season and Advanced sat on "Loading advanced stats…" until opening night.
...
- League page (Scoreboard — the default tab on all 4 leagues — Standings, Bracket, Leaders, Power Rankings, Draft)
...
- Injury details (2026-09): the `/injuries` rows now carry `injury_type`/`injury_side`/`injury_detail`/`return_date` (from ESPN's per-entry `details` object — see `eyewall-pipeline`'s `injuries.py`). The badge and its rendering live in the shared `components/InjuryBadge.jsx` (`InjuryBadge`, `InjuryDetailTip`, `InjuryDetailLine`), with pure formatting/sorting helpers in `utils/injuryDetails.js`. Surfaces: an ⓘ tip beside every Scouting-tab badge ("Knee · Surgery · Est. return Sep 20 · Updated Sep 10"), an injury banner in the player popup (both roster and league context — skipped when the player's team is unknown rather than guessing), and an Injury report card on the Team page's Overview tab (worst-first: IR, out, day-to-day, suspension). ESPN's `return_date` is a bare `YYYY-MM-DD` and is parsed as local midnight (`parseLocalDate()`), not UTC, so it doesn't render a day early in North American time zones. An entry whose estimated return date has already passed is flagged "may be out of date", since ESPN's feed is known to keep healed injuries around. Badge labels are now translated (EN `DTD/OUT/IR/SUSP`, FR `JÀJ/ABS/LB/SUSP`); ESPN's body-part/detail strings stay untranslated, per the localization policy for external API values.
- Transactions tab on the News page (2026-09): `TransactionsFeed.jsx` now takes a `sport` prop — `"pwhl"` (default, unchanged HockeyTech feed on `PWHLNewsView`) or `"nhl"` (`NewsView`), sharing one header/loading/error/empty shell. The NHL feed reads the Worker's `/transactions` route via `getTransactions(scope, team)` (backed by `eyewall-pipeline`'s nightly ESPN ingestion — the NHL API has no transactions endpoint), toggles between the selected team and the whole league, shows a category badge (trade, signing, waivers, injury, recall, assignment, release, suspension, staff) with ESPN's own description text, and renders each trade — already paired from its two per-team ESPN entries by the Worker — as one card with both sides. Not memoized client-side (the Worker's 1hr KV cache is the cache). Covered by `cypress/e2e/nhl-transactions.cy.js`, which stubs `/transactions`.
- Trade trees (2026-09): every NHL trade in that feed (paired, or a single trade entry whose other half ESPN never posted) has a "Trade tree" toggle that opens `TradeTree.jsx` inline — what each team received and, for every asset traded again, what its new team got back, recursively. Picks show the player they became once drafted, or say why not (`not drafted yet` / `not traced` / `can't tell which pick` — never guessed); "How they got here" lists the earlier trades that brought in what this one sent out. Any name with an NHL player id — a traded player, or the player a pick became — opens that player's `PlayerPopup` (looked up through the Worker's `/player/landing`, as `MilestonesFeed` does; `feature_interacted { feature: 'trade_tree', action: 'player_open', source: 'traded' | 'drafted' }`); names without one stay plain text. Reads the Worker's `/trades/tree?tx=` via `getTradeTree(txId)` (backed by `eyewall-pipeline`'s nightly `trade_trees.py`, built from ESPN's trade text and the NHL's draft records). Covered by `nhl-transactions.cy.js`, which stubs `/trades/tree`.
- Feature usage events (2026-09, `utils/analytics.js`): `feature_viewed { feature }` fires once per mount when a card is at least half on screen (`useFeatureViewed()`, an IntersectionObserver callback ref), and `feature_interacted { feature, action }` on taps (`trackFeature()`), for `playoff_odds`, `injury_impact` (`show_all`), `probable_starters` (`gameId`), `scorecard` and `trade_tree` (`open`; the view carries `found`, `trades`, `origins`, `truncated`). `info_tip_opened { label, lang, path }` records which explainers get read. Every event carries `team` (the selected NHL team) and `environment` as PostHog super properties (`main.jsx`). As before, nothing is sent outside production builds. Unit-tested in `src/utils/__tests__/analytics.test.js`.
- Scratches (2026-09): a Scratches card on the Team page's Overview tab (below the injury report) and a "Scratched N games in {season}" line in the player popup, both from the Worker's `/scratches` route via `getTeamScratches(team)` (backed by `eyewall-pipeline`'s nightly `scratches.py` — the NHL's own game-roster scratch lists, every team). The card lists the selected team's players most-scratched first (top 8, "Show all"), with the last scratch date; the popup uses the player's own team. Before a team's first regular-season game the Worker falls back to last season, which the card labels ("No regular-season games yet — showing 2025-26") rather than hiding. The healthy/injured/suspended split (H / Inj / Susp chips, "N healthy" in the popup) only appears once scratches have actually been classified against the injury report — every game before 2026-09-12 is unclassified, so last season shows totals only, with a note saying so. Covered by `cypress/e2e/team-scratches.cy.js`, which stubs `/scratches`.
- Recent drafts on the Picks tab (2026-09): a "Recent drafts" card below the 2026 draft card, from the Worker's `/draft/pick-history` route via `getDraftPickHistory(team)` (backed by `eyewall-pipeline`'s nightly `draft_history.py` — the NHL's own draft records, last 5 drafts, every team). Year chips (newest first), a summary line ("26 of 40 picks made came from trades · 20 own picks traded away (2022–2026)"), the picks the selected team made with where each came from ("via UTA → CGY" — every team that owned it first), and the team's own picks that other teams used ("→ PIT (via NSH)"). Rows aren't clickable (`DraftPopup` is built around the live draft-day tables). Covered by `cypress/e2e/team-pick-history.cy.js`, which stubs `/draft/pick-history`.
- Playoff odds on the Overview tab (2026-09): a Playoff odds card below the record blocks, from the Worker's `/playoff-odds` route via `getPlayoffOdds(team)` (backed by `eyewall-pipeline`'s nightly `playoff_odds.py` — the rest of the regular season simulated 10,000 times from team Elo ratings, every team). Shows the chance to make the playoffs, win the division, and projected points (10th–90th percentile range); the whole division from that same run, ranked, with the selected team highlighted ("2 of 8 in the Metropolitan") so the win-division number reads as a standing rather than a bare percentage; a season trend sparkline; why the number moved since the last run ("▲ 6.1 pts since Oct 14", then each finished game's effect — "CAR beat OTT +4.0" — and what those games don't explain: rating changes and games further out); and the next game day's stakes ("OTT @ CAR — OTT win: 57% · CAR win: 66%", the team's own games plus the other games that swing its odds most). Plain probabilities, no betting framing. Labeled "Early season" before 20 games played (the pipeline's backtest shows preseason odds barely beat a coin flip), and "Not updated since …" once the nightly stops writing (season over) rather than hidden. Before the season's first run the card says odds will appear after it. Covered by `cypress/e2e/team-playoff-odds.cy.js`, which stubs `/playoff-odds`.
- Games lost to injury on the Overview tab (2026-09): a card under the Injury report, from the Worker's `/injury-impact` route via `getInjuryImpact(team)` (backed by `eyewall-pipeline`'s nightly `injury_impact.py`, every team). The Injury report is who's hurt now; this is what injuries have cost so far this season: man-games lost and WAR lost, each with its league rank ("#5 of 32", 1 = most lost), players injured, the league average for context, and the injured players most-games-missed first (injury type, last game missed, games missed, WAR lost; top 6, "Show all"). A man-game lost is a game a player on the day's injury report didn't dress for, valued at his WAR per game (this season and last pooled); goalies count in games only (no goalie WAR). Injury history starts 2026-09-12, so the card says tracking starts with the first regular-season game until then. Covered by `cypress/e2e/team-injury-impact.cy.js`, which stubs `/injury-impact`.
- Probable starters in the game preview (2026-09): a "Probable starters" block in `MatchupDetail`'s Prediction tab (both render paths — including the no-standings path that covers preseason and opening night), from the Worker's `/probable-starters` route via `getProbableStarters(gameId)` (backed by `eyewall-pipeline`'s nightly `starting_goalie.py` — a model of each team's own pattern: share of recent starts, who started last game, back-to-backs, rest; the NHL announces no starters ahead of time, and the model picks the starter ~73–75% of the time in its backtest). `components/ProbableStarters.jsx`: one column per team, the selected team first — the most likely starter with his probability and share of the last 10 starts, "Started last game" / "Back-to-back" / "Day-to-day" tags, and the other goalies smaller. Posted once the game is within 2 days (the block says so before then); renders nothing for preseason, playoff, or finished games, which aren't modeled. Plain probabilities, no betting framing. Team labels use `teamConfig.js`'s `teamTextColor(abbr)` (see "Readable team colors" below).
- Readable team colors (2026-09): every other-team color in the app — opponent names/scores/logo fallbacks on game cards, the game preview, game stats popup, calendar, Scouting tab, Shot Map, Team page and Schedule playoff series, the Team Picker's hover accents, and the League page's standings, leaders and bracket — now comes from `teamConfig.js`'s `teamTextColor(abbr)`: each team's WCAG AA `displayColor` on dark, `primaryColor` on light, the same rule `applyTeamTheme()` already used for the selected team's `--team-primary`. Before this, those places read `nhlApi.js`'s `TEAM_COLORS`, a raw brand-color map from the initial commit that Light Mode (2026-06) never migrated — FLA/WPG's `#041e42` and the other blue primaries were unreadable as text on the dark card. That map is removed. The League page had its own local `displayColor` map since the bracket styling work, but it used the dark-mode color in light mode too (FLA's `#5b9ef9` is ~3:1 on white); it now follows the theme like everything else. `teamTextColor()` returns null for an abbr not in `ALL_TEAMS` (e.g. ARI in older seasons), so each call site's existing fallback color still applies.
- Projected lines in the Scouting tab (2026-09): a "Projected lines" block above the season's line combinations, from the Worker's `/projected-lines` route via `getProjectedLines(team)`. It's backed by `eyewall-pipeline`'s nightly `projected_lines.py`: last game's 5v5 pairings in-season, minus injury-report outs and players off the roster; before a team's first game, pooled preseason pairings ranked by last season's ice time.
  - **Accuracy line**: one line whose copy depends on the basis (`utils/projectedLines.js`'s `projectionCopyKeys()`), from the pipeline's backtest before the lineup is known: about 7 in 10 forward linemate pairs and 3 in 4 D pairs in-season, about half and 6 in 10 on opening night.
  - **Fill-ins**: players who replace someone from the basis lineup get a "fill-in" tag with a tooltip.
  - **Injuries**: badges match by player id, since these rows carry ids.
  - **Hidden** when there's no projection yet (`hasProjection()`), with no empty state.
  - **Renamed section**: the existing section is now "most-used lines this season", so the two aren't confused.
  - **Tests**: `cypress/e2e/projected-lines.cy.js` (stubs the schedule and `/projected-lines`) and `src/utils/__tests__/projectedLines.test.js`.

- Prediction scorecard + Elo everywhere (2026-09): a **Scorecard** tab on the League page (`components/PredictionScorecard.jsx`) from the Worker's `/scorecard` route via `getScorecard()` (backed by `eyewall-pipeline`'s nightly `prediction_scorecard.py`): for game winners, starting goalies and playoff odds, the **live record** (only predictions published before the fact, from 2026-27 — "pending" until something's graded) and a **backtest** (the model replayed on past seasons, always labeled as one), each with accuracy, Brier score, the baseline it has to beat, calibration ("said 56% → happened 60%") and, for live records, the latest graded picks. When accuracy barely beats the baseline but Brier clearly does (playoff odds' backtest: 71.6% vs 71.4%, Brier 0.162 vs 0.224) the card says the edge is in the percentages, not the picks. **The game preview's win bar, the schedule's favoured chips and the locally auto-saved track record now all show the Elo win probability** (`utils/eloWinProb.js`'s `teamWinPct()`, fed by the Worker's `/elo/ratings` via `getEloRatings()`) — the exact formula the Worker's `/prediction/analyze` and the pipeline's `win_probs.py` use, so what the app shows is what the scorecard grades. They used to show `computeWinPct()`, a hand-tuned standings formula blended 60/40 with the sportsbook's implied odds; that formula is removed. The DraftKings odds row in the preview is still shown as-is, separately. Plain probabilities, no betting framing. Covered by `cypress/e2e/league-scorecard.cy.js` (stubs `/scorecard`) and `src/utils/__tests__/eloWinProb.test.js`. Covered by `cypress/e2e/probable-starters.cy.js`, which stubs the team schedule (a trimmed real 2026-27 CAR schedule, `cypress/fixtures/schedule-car-2026-27.json`) and `/probable-starters`.
- How it works page (2026-09): `/methodology` (`views/MethodologyView.jsx`), linked from the scorecard ("How these models work →") and back to it (`/league?tab=scorecard` — `LeagueView` now opens the tab named in `?tab=`). Ground rules, then how game winners (Elo), starting goalies and playoff odds make their predictions, how the scorecard grades them (picked right, Brier, calibration, live vs backtest), the method behind injury impact and trade trees, data sources and the nightly schedule, each with what the model can't see; plus a dated **changelog** (`CHANGELOG` in the view) so a prediction can be matched to the model version that made it. Settings in the text were checked against `eyewall-pipeline`'s code on the date shown (`METHOD_CHECKED`); each model's backtest line comes live from `/scorecard`, not typed in. Every section has an anchor (`/methodology#playoff-odds`) and fires `feature_viewed { feature: 'methodology', section }`. en/fr. When a model changes, update its section and add a changelog entry. Covered by `cypress/e2e/methodology.cy.js` (stubs `/scorecard`).
...
- Lock Screen live score (iOS app): Settings → Lock Screen → **Follow {team} games** (iOS 17.2+, shown only there and only for an NHL favorite). With it on, eyewall-poller starts an ActivityKit Live Activity at puck drop via push-to-start, whether or not the app is open, keeps it updated, and ends it at the final. Native `LiveActivityRegistrar` (`ios/App/App/SceneDelegate.swift`, started from `AppDelegate`) reports the push-to-start token (`POST /live-activity/start-token`) and each activity's update token (`/live-activity/register`), including on the background launch iOS gives the app after a server start. `hooks/useLiveActivity.js` re-syncs the team and language on every launch, and starts the activity from the app when it's open during the game and none exists yet (once per game, so a swiped-away one isn't brought back). The per-game "Follow on Lock Screen" button above the live rink was removed in favor of the setting.
...
- **NHL season selector only offers ingested seasons (2026-09):** the shot map's season chips and "More seasons" dropdown list 2026-27 back to 2022-23, not back to 2013-14 as they did before. The pipeline has never ingested `shot_events`, `shift_events`, `player_seasons` or `team_seasons` for anything earlier, so those entries rendered a schedule and game chips (which come from the NHL API, and really do go back) next to an empty shot map, an empty Players table and no PP/PK units — with nothing on screen explaining why. Backfilling is the alternative and stays open: `nhl_stats.py`, `shot_events.py` and `shift_data.py` each take a season argument, and `NHL_ARCHIVE_SEASONS` in `teamConfig.js` is the only thing to change once a season lands. Budget ~1M `shift_events` rows per season, on a table that already returns a Postgres statement timeout on a full `COUNT`.
- **PP/PK unit chips were invisible until 2026-09:** the shot map's special-teams drill-downs read unit rosters from `ppUnits.js`'s static constants long after that data moved to Supabase's `special_teams_units`. The constants were emptied but kept as a compatibility shim, and nothing failed — an empty map and a missing map both just render no chips. Now wired to `getSpecialTeamsUnits(season)` and covered by Cypress. 2026-27 has no rows until games are played; that is the season not having happened, not a gap to fix.
...
- [x] French/English localization — **Track B follow-up: AI predictions (2026-09).** Pre-game predictions were the one AI narrative Track B missed, so they stayed English in French. `game_predictions` now has a `locale` column and `ai_predictions.py` generates both languages (eyewall-pipeline). The Worker's `/game-predictions` and all four on-demand AI prediction routes (`/prediction/analyze`, `/pwhl/prediction`, `/ahl/prediction`, `/echl/prediction`) take `?locale=` and cache French under a `:fr` key (eyewall-poller). Here: `getGamePrediction()`/`getGameMatchup()`, `fetchPWHLPrediction()`/`fetchAHLPrediction()`/`fetchECHLPrediction()` and the Worker `/cache/` lookup (`predictionCacheKey()`) all send `i18n.language`. `MatchupDetail.jsx`'s AI section, `PredictionShareCanvas.jsx`, `ScoutingTab.jsx`'s matchup analysis and the PWHL/AHL/ECHL preview popups refetch when the language changes, and drop a response that arrives after a later switch. Also fixed along the way: `PredictionShareCanvas.jsx`'s cache fallback looked up `prediction:{gameId}` without the team, a key the Worker stopped writing when it went multi-team. Still English-only: the other on-demand AI routes (`/summary/narrative`, `/pwhl/scout`, head-to-head narratives, draft analysis).
