...
### Pending
- [ ] French/English localization (Phase A0 + A1 + eleven A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, and `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx` are all fully translated. This sub-PR added `MatchupDetail.jsx` (NHL's expandable pre-game matchup panel — win probability, predicted score, edge-factor checklist, top-line card, EyeWall AI analysis chrome) and `PWHLGamePreviewPopup.jsx` (the PWHL sibling — a full popup with season series, head-to-head, team form, hot streaks, team leaders, special teams). The two files' identical "📊 X/Y correct (Z%)" track-record line was consolidated into a single shared `common.trackRecordLine` key rather than duplicated. Also fixed `PWHLGamePreviewPopup.jsx`'s hardcoded English `MONTH_NAMES` array (same bug class fixed twice already in `PWHLScheduleView.jsx`/`CalendarView.jsx`/`PWHLCalendarView.jsx`) by routing through `formatters.js`'s `formatDate()` Intl wrapper. "⚡ EyeWall AI" is left as a literal brand name, not translated. Verified via Cypress (both files' Cypress coverage, including the exact "renders season series, head-to-head, team form, hot streaks, leaders, and special teams" test, passes) rather than a full live click-through — no upcoming PWHL games exist in the current season's local dev data to open the popup with. The rest of the app's ~36 remaining view/component files still need string extraction (long tail: `GameStatsPopup.jsx`+`PWHLGameStatsPopup.jsx` — the post-game box-score popups, structurally similar to this sub-PR's pre-game pair — plus TriviaFeed, StatTileGrid, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia — including this file pair's own AI prediction narrative text, which is served pre-generated and untouched by this UI-chrome-only phase) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
