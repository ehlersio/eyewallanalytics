...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-three A2 sub-PRs done, `PlayerPopup.jsx` fully translated, `PWHLPlayerPopup.jsx` underway) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, and `PlayerPopup.jsx` are all fully translated. This sub-PR (1 of N for `PWHLPlayerPopup.jsx`) covers everything outside the main popup export: `PWHLRadarChart`'s "not enough playing time" note (reused NHL's `playerPopup.radar.notEnoughData` key byte-for-byte), both the goalie and skater Heat Map tabs (empty/loading states, summary chip labels, dot/zone toggle buttons, filter buttons, shooter-perspective caption — nearly all reused NHL's existing `playerPopup.heatMap.*` keys, since this file's heat map is structurally the same feature; minted PWHL-only keys only where wording genuinely differs, e.g. `heatMap.skater.emptyPwhl` and a new `heatMap.loading` for PWHL's own useFetch-driven loading state that NHL's sibling doesn't have), and the on-demand AI Scout tab (`PWHLScout` — a "Generate Report" button flow, functionally different from NHL's pre-generated `ScoutingBlurb`, so its prompt/button/failure/retry/footer strings got a new `pwhlScout` namespace; reused `scoutingBlurb.header` for the byte-identical "Scouting Report" label). Live testing on the PWHL side surfaced a real bug that was **also already live in the shipped `PlayerPopup.jsx`** (merged last sub-PR): the goalie heat map's per-zone save-percentage overlay had a hardcoded `{z.total} shots` string inside the SVG that neither sub-PR's translation pass caught, because it's data-driven overlay text with no French equivalent player having enough zone data to trigger it during the NHL sub-PR's live check. Found it live on a PWHL goalie (Aerin Frankel) with real zone data, fixed both files with a shared `playerPopup.heatMap.goalie.zoneShotsCount` key, and confirmed via live re-test on both the PWHL goalie (now "198 tirs") and re-ran the full test suite — a rare case of a French-only sub-PR fixing a bug in an already-merged, previously-believed-complete file. The main `PWHLPlayerPopup` export (bio grid, tabs, Stats/Compare tabs, section headers) remains untranslated for a future sub-PR — confirmed still English live (HEIGHT/WEIGHT/SHOOTS/AGE/BIRTHDATE/HOMETOWN, "Career Regular Season", tab labels, "Select two or more seasons above to compare.", etc.). All 34 `playerPopup.*`/cross-namespace keys used in this file audited to resolve in both locale files; ESLint and the full vitest suite (185/185) clean; `pwhl-players.cy.js` (89/89) and `players.cy.js` (72/72, after ruling out an interleaved dev-server crash as the session's known local-sandbox instability) both green. The rest of the app's ~24 remaining view/component files still need string extraction (long tail: the rest of `PWHLPlayerPopup.jsx` and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
