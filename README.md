...
### Pending
- [ ] French/English localization (Phase A0 + A1 + eight A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, and `ShotMapView.jsx`+`PWHLShotMapView.jsx` are all fully translated. This sub-PR added `components/GameEvents.jsx` + `components/PWHLGameEvents.jsx` — the game-event popups (Goal/Penalty/Win/PuckDrop/HatTrick) shared across both shot map views, plus PWHL's `PWHLLiveInsights`/`PWHLInsightsCard` (the PWHL sibling of `ShotMapView.jsx`'s already-translated `LiveInsights`, ~10 of its ~15 insight strings reuse the identical NHL `shotMapView.liveInsights.*` keys, the rest got PWHL-specific `pwhlGameEvents.liveInsights.*` keys where wording differs). Also fixed a real bug found while translating: PWHL's `finalLoss` insight had a confusing/broken phrase ("outshot BOS 45-30" instead of "BOS outshot them 45-30") — restructured to reuse NHL's clearer `finalLoss`/`outshootingClause`/`outshotClause` pattern instead of preserving the original ambiguous wording. Both files' 5-tap debug panels remain deliberately English-only (dev-only synthetic test fixtures, confirmed isolated from the real translated code paths during verification). The rest of the app's ~43 remaining view/component files still need string extraction (long tail: ScheduleView, TriviaFeed, StatTileGrid, popups, etc.), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
