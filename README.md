...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty A2 sub-PRs done, `PlayerPopup.jsx` underway in sub-parts like `ShotMapView.jsx` was) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, and `PWHLPeriodSummary.jsx` are all fully translated. `PlayerPopup.jsx`'s header/radar area and Heat Map tab were done in the previous sub-PR; this sub-PR (2 of N) covers `PlayerAnalytics` (both skater and goalie Analytics tabs, all WAR/GSAX context chips and their InfoTip tooltips, all percentile-bar labels), `ResultsVsProcess`, and `ScoutingBlurb` — the main popup body (Stats tab, bio/contract panel, rankings section, tab shell) remains for a future sub-PR. Also fixed `ScoutingBlurb`'s `fmtDate()` hardcoded `'en-US'` date formatting, the same bug class fixed repeatedly across this migration, now routed through `formatters.js`. Live testing surfaced a real bug in a file this sub-PR doesn't otherwise touch: `PercentileBar.jsx` (a small shared component also used by `PlayerComparisonPopup.jsx`) had the identical hardcoded-`'th'`-suffix bug already fixed twice this migration (`StatTileGrid.jsx`, then `PlayerPopup.jsx`'s own `RankBadge` last sub-PR) plus five untranslated tier labels ("Elite"/"Great"/"Above avg"/"Below avg"/"Poor") and an "N/A" fallback — fixed it too, in scope, since it was directly blocking correctness of the very Analytics tab percentile bars this sub-PR translates (confirmed live: "88th"/"Great" was still showing in French before the fix, "88e"/"Excellent" after). Verified extensively live in French on both a skater (Sebastian Aho: WAR card, all 8 context-chip labels, all 10 percentile bars, Results vs. Process section) and a goalie (Pyotr Kochetkov: GSAX card, all 4 context-chip labels, all 6 percentile bars) — every ordinal marker and tier label confirmed correct post-fix — plus the Scout tab's empty state with the player name interpolated correctly. `players.cy.js` (72/72 executed, 0 failing) run twice for confidence, after ruling out an interleaved dev-server crash as the session's known local-sandbox instability under Cypress load. The rest of the app's ~24 remaining view/component files still need string extraction (long tail: the rest of `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
