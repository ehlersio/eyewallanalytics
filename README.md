...
### Pending
- [ ] French/English localization (Phase A0 + A1 + seven A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js` (localStorage-backed, modeled on `themeConfig.js`), `localeSync.js` (cross-device sync for signed-in users via `user_preferences.preferred_locale`, modeled on `favoriteTeamSync.js`), Settings-popup Language toggle. Global chrome translated (`BottomNav.jsx`, `Topbar.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `NotificationBell.jsx`, `AccountSection.jsx`), plus `LeagueView.jsx` + `PWHLLeagueView.jsx` (Standings/Bracket/Leaders/Power Rankings/Draft), `TeamView.jsx` + `PWHLTeamView.jsx` (Overview/Advanced/Splits/Trends/Cap-Salaries/Picks), `PlayersView.jsx` + `PWHLPlayersView.jsx` (Roster/Stats), and `ShotMapView.jsx` + `PWHLShotMapView.jsx` — now **fully translated** aside from the 5-tap debug panel (deliberately English-only, dev-only tooling never seen by real users). This final sub-PR covered `LiveInsights`'s ~34 templated insight strings (period dominance, momentum runs, PK performance, coach's challenges, faceoff dominance, scoring droughts, head-to-head record, live score situation, final result, empty net), `LiveInsightsCard`'s header, `MomentumCard`, `AdvancedGamePanel`, and PWHL's `ShotAttemptsPanel` (its `AdvancedGamePanel` equivalent, found while working this area — PWHL's PDO/Luck help text is fully computed with live values rather than NHL's static explainer, so those got their own `pwhlShotMapView.advanced.*` keys). Also fixed a pre-existing bug found while translating: the empty-net insight had a hardcoded `'CAR goalie pulled'` literal instead of using `TEAM_CONFIG.abbr`, meaning every other NHL team's shot map showed the wrong abbreviation there — it now correctly interpolates the selected team. Note: PWHL's live-insights equivalent (`PWHLLiveInsights`) lives in `components/PWHLGameEvents.jsx`, a shared file outside this pair's scope — not covered by this sub-PR, tracked separately in the long tail below. Shared `league.*`/`team.*`/`players.*`/`shotMapView.*` i18n keys reused across NHL/PWHL file pairs where the English text was byte-identical; spelling/casing/wording variants that differ deliberately kept as separate keys rather than force-merged. The rest of the app's ~45 remaining view/component files still need string extraction (next: `components/PWHLGameEvents.jsx`'s `PWHLLiveInsights`, then the long tail), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
