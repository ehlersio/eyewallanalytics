...
### Pending
- [ ] French/English localization (Phase A0 + A1 + thirteen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, and `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx` are all fully translated. This sub-PR added `TriviaFeed.jsx` (the Daily Trivia tab, shared between NHL/PWHL, not a sibling-file pair like most of this migration) — header title/stats line, tier badges (Easy/Medium/Hard), per-tier empty states, and the Correct/Incorrect result badge and error/retry UI. The question text, answer options, and explanation are AI-generated content served from the API and correctly left untouched (Track B scope, still pending). One correctness fix beyond simple extraction: the original code built its per-tier empty-state message by lowercasing the tier's own display label (`` `No ${label.toLowerCase()} question...` ``) — reused verbatim, that would have produced ungrammatical French (`moyen` needs to agree with the feminine `question` as `moyenne`). Split into three fully independent per-tier keys instead of one interpolated template, so each language can inflect correctly. Verified live in French (real trivia data, all three tiers, including answering a question to confirm the result badge and stats-line update) and via Cypress (`trivia.cy.js` 7/7, `read-state-badges.cy.js` 7/7, both green). The rest of the app's ~33 remaining view/component files still need string extraction (long tail: `StatTileGrid.jsx`, `ScoutingTab.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`, `PeriodSummary.jsx`+`PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
