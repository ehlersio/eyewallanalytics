...
### Pending
- [ ] French/English localization (Phase A0 + A1 + nine A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, and `GameEvents.jsx`+`PWHLGameEvents.jsx` are all fully translated. This sub-PR added `ScheduleView.jsx`+`PWHLScheduleView.jsx` (headers, tabs, playoff round labels, series-status badges, sort bar, empty states, result/upcoming cards). Also wired `PWHLScheduleView.jsx`'s date formatting through the previously-unused `formatters.js` `formatDate()` Intl wrapper (built in Phase A0 but not adopted anywhere until now) — its `MONTH_NAMES`/day-of-week arrays were hardcoded English, so French was rendering "Nov 21" instead of a real French date; now renders real Intl-formatted dates (e.g. "sam. 25 avr." / "jeu. 30 avr."). One remaining gap, left as-is rather than risking a data-flow change: when the API's `game_date` field is absent, the code falls back to the API's own pre-formatted English `date_with_day` string ("Fri, Nov 21") verbatim — that fallback path (rare) still shows English regardless of locale, since it has no year and can't be reliably re-parsed into a real Date. The rest of the app's ~41 remaining view/component files still need string extraction (long tail: `components/GameCard.jsx` — the shared game-card/`SortBar`/`MatchupDetail` component both `ScheduleView.jsx` and `PWHLScheduleView.jsx` render games through, still English-only — plus `CalendarView.jsx`, TriviaFeed, StatTileGrid, popups, etc.), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
