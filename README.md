...
│   │   ├── LiveEventRink.jsx           # Live-game-only compact rink (Session 100) — plots shots/hits/faceoffs/giveaways/takeaways as dots that fade over real elapsed time since first observed (not a hard count/window cutoff), goals pinned/no decay. Independent coordinate-transform copy from IceRink.jsx (not a cross-import), normalized via each play's own `homeTeamDefendingSide` so home's net is always on the left, stable across period parity. Intermission: tracked events clear and a continuous CSS-only Zamboni resurfacing animation (`@keyframes zamboni-drive`, index.css) runs for the whole break, overlaid with the same ordinal+countdown treatment the score bar already shows. NHL-only for now — PWHL's HockeyTech feed lacks penalty coordinates and has no giveaway/takeaway event type at all, scoped as a separate follow-up
