...
### Pending
- [ ] French/English localization (Phase A0 + A1 + seventeen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, and `NewsView.jsx`+`PWHLNewsView.jsx` are all fully translated. This sub-PR added `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx` (the 1080×1080 export/share cards for the pre-game prediction popups) plus `ShareButtons.jsx`, the shared Save Image/Post to X/Share button row both cards use — translated as part of this pass since it's directly consumed here (still has 3 other untranslated consumers — `PeriodSummary.jsx`/`PWHLPeriodSummary.jsx`/`ScoutingTab.jsx` — which will pick up French for free once each is translated in turn). Reused `pwhlGamePreview.prediction.sectionLabel`/`.streakLabel`/`.shotAttemptShareLabel`/`.shotAttemptCaveatPrefix` and `gameStatsPopup.summary.badge` ("⚡ EyeWall AI", already an established untranslated-brand-name precedent) rather than duplicating byte-identical strings from the MatchupDetail/PWHLGamePreviewPopup sub-PR. The X/Twitter share-caption text (headline + win-probability lines) was translated too, not just the on-canvas UI — a French user posting a prediction card should get a French caption. Position-code abbreviations (LW/C/RW) and stat abbreviations (xGF%) were left as-is, consistent with this migration's stat-code policy. Verification hit the same live-data gap as two prior sub-PRs: neither NHL nor PWHL has any upcoming games in this dev environment (both leagues' 2026-27 seasons haven't started), so the popups these cards render inside can't be opened by hand right now — verified instead via Cypress, including the exact tests that exercise `ShareButtons.jsx`'s post-edit rendering across all its live consumers (`pwhl-schedule.cy.js`'s "shows Save Image / Post to X share buttons for the prediction card" 34/34, `period-summary.cy.js`'s "shows Save Image and share buttons" 29/29, `league.cy.js`'s power-rankings export coverage 34/34 executed — all green, no regressions from the `ShareButtons.jsx` edit). The rest of the app's ~27 remaining view/component files still need string extraction (long tail: `ScoutingTab.jsx`, `PeriodSummary.jsx`+`PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
