...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-nine A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `App.jsx`, `InfoTip.jsx`, `SeasonTypeToggle.jsx`, `SeasonChipRow.jsx`, `GameChipsRow.jsx`, `TeamOpponentPicker.jsx`, `SeasonOverlayChart.jsx`, `LiveEventRink.jsx`, `PlayerComparisonEntry.jsx`, `SeasonComparisonPicker.jsx`, `TeamPicker.jsx`, and `PlayerComparisonPopup.jsx` are all fully translated. This sub-PR covers `TeamComparisonPopup.jsx` — the team-level "Compare Seasons"/"Compare Teams" popup opened from `TeamView.jsx`/`PWHLTeamView.jsx`, the last of the app's ~660-line popups. Covers both modes: vs-Season (season picker empty states, the xGF% overlay chart section reusing `teamView.advanced.xgfSparklineTitle` and `playerPopup.compareTab.selectSeasons`/`.loadingChart` byte-for-byte) and vs-Team (the opponent-picker empty states, the Full Stat Comparison / Head-to-Head submode toggle, and the Head-to-Head scoreboard — win-count labels, "Last N" and win-streak pills, the thin-sample caveat using this migration's now-second `_one`/`_other` plural pair, and the AI narrative card reusing `gameStatsPopup.summary.badge` and `periodSummary.ai.generating` byte-for-byte). The header's dynamic title (`"{{team}} vs {{opponent}}"` once an opponent is picked, else "Compare Teams"/"Compare Seasons") and the mode-switch/submode-switch buttons and their aria-labels are also translated. Found and fixed two more latent `t`-shadowing hazards in this file (`opponentOptions`' two `.map(t =>` calls and `opponentLabel`'s `.find(t =>`, all renamed to `team`/`opt`) before they could cause a bug, plus translated a real-but-likely-dead `` `Season ${val}` `` fallback string (also present, unfixed, in `PlayerPopup.jsx`/`PWHLPlayerPopup.jsx` from earlier sub-PRs — left alone there since it's the same low-priority, probably-unreachable defensive fallback, not worth reopening two shipped files for). All 32 keys used (including the plural pair) audited to resolve in both locale files; ESLint and the full vitest suite (185/185) clean; `team.cy.js` (65/65) and `pwhl-team.cy.js` (128/128) both green. Live-verified in French on both modes: season comparison (xGF% chart section title and x-axis, season cards), team comparison (opponent picker, submode toggle, the "{{team}} vs {{opponent}}" header, and a real Head-to-Head result — "Depuis 2023-24", "victoires", a correctly-interpolated win-streak pill, and the "⚡ EyeWall AI" narrative card). **This closes out every popup/view in the app's component tree except `DraftTab.jsx`** — the last remaining file, and the biggest single one left (~586 lines / 30+ strings). `DevReplayView.jsx`/`PWHLDevReplayView.jsx`/`DevDraftView.jsx` are dev-only routes gated behind `import.meta.env.DEV` and never ship to real users, so they stay deliberately out of scope. AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) still needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
