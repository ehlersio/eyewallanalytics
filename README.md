...
### Pending
- [ ] French/English localization (Phase A0 + A1 + eighteen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, and `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx` are all fully translated. This sub-PR added `ScoutingTab.jsx` (NHL-only pre-game scouting report — season/playoff comparison, goalie matchup, team total projection, recent form dots, top skaters/goalies, line combinations, and its own 1080×1080 share canvas), the largest single file translated so far this migration (~90 keys). `ShareButtons.jsx` picks up French here for free (2nd of its 3 untranslated consumers now translated — only `PeriodSummary.jsx`/`PWHLPeriodSummary.jsx` remain). Found and fixed a real bug shared across two files: `computeGSAx()` in `advancedStats.js` (used by both this file and the earlier-shipped `GameStatsComponents.jsx`) returned a hardcoded English `.note` string — routed it through `i18n.t()` directly in the utility (pure JS, no React needed — `i18n` is the raw i18next instance, not the hook), so both consumers get the fix. Matched an already-shipped precedent from `TeamView.jsx`'s identical form-dot pattern: bare W/L/O result letters stay untranslated (only the tooltip is), rather than inventing a new convention. Position-code and stat-code abbreviations (LW/C/RW, xGF%) left as-is per this migration's established policy. Verification was unusually constrained: `MatchupDetail.jsx` gates its Prediction/Scouting tabs behind season standings, which don't exist yet this early in the 2026-27 preseason even though upcoming games do — so the tab can't be opened by hand at all right now, and every Cypress test exercising it is `liveSeriesIt`-gated (54 pending in `schedule.cy.js`, 0 failing — confirmed no regressions in what currently runs). Relied instead on a full manual audit: every one of the ~90 `t()` calls' keys checked programmatically against both locale JSON files to confirm they resolve, on top of the usual ESLint/vitest passes. The rest of the app's ~26 remaining view/component files still need string extraction (long tail: `PeriodSummary.jsx`+`PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
