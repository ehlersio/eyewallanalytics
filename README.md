...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-four A2 sub-PRs done, `PWHLPlayerPopup.jsx` now fully translated) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`, and now `PWHLPlayerPopup.jsx` are all fully translated. This sub-PR (2 of 2 for `PWHLPlayerPopup.jsx`) covers the main popup export: the 6-field bio grid, the non-reflowed header's inline shoots/catches chip and birth+age+hometown line, the close button (reused `common.close`, byte-identical to PWHL's shorter "Close" text vs. NHL's "Close player details"), the 4-tab toggle, the Stats tab (loading state, current/career section headers, empty state), and the Compare tab (empty state, per-game trend label/select, loading state, per-season no-data message). Fixed `fmtBirth()`'s hardcoded `'en-US'` date formatting via `formatDate()`, the same bug class fixed repeatedly across this migration. Minted PWHL-specific keys only where source text genuinely differs from NHL's sibling despite meaning the same thing — e.g. PWHL's section headers capitalize "Regular Season" throughout where NHL's use "Regular season," so `sections.seasonRegularPwhl`/`careerRegularPwhl` preserve that existing casing rather than silently normalizing it (French text is identical either way, since French doesn't inherit the English capitalization quirk). Live-verified extensively in French on Susanna Tapani (bio grid incl. French month name "2 mars 1993", tabs, current/career/playoffs section headers, Compare tab's per-game trend chart) — confirmed correct across the picker's league/team-selection flow needed to reach a PWHL roster from a clean locale switch. All 59 `playerPopup.*`/cross-namespace keys used in this file audited to resolve in both locale files; ESLint and the full vitest suite (185/185) clean; `pwhl-players.cy.js` (89/89) and `players.cy.js` (66/66, NHL vitest scope) both green after ruling out one interleaved failure in each spec as the session's known local-sandbox instability (confirmed via clean reruns, not a regression). This closes out `PWHLPlayerPopup.jsx`. The rest of the app's ~24 remaining view/component files still need string extraction (long tail: other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
