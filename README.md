...
- [ ] French/English localization — **Track B (AI-generated narrative content) not started.** Scouting blurbs, results-vs-process narratives, game summaries, and trivia are all generated once in English by `eyewall-pipeline` with no locale dimension anywhere in the data model. Needs (per the original plan): a `locale` column added to `game_summaries`/`player_scouting`/`player_narratives`/`trivia_questions` with each table's upsert conflict key extended to include it (Phase B0); `ai_persona.py`'s prompt-builders and the pipeline scripts' `generate()` call sites threaded with a `locale` parameter, generating both `en` and `fr` going forward only — no historical backfill (Phase B1); and `eyewall-poller` route changes to accept a `lang` query param and cache French/English responses independently in KV (Phase B2). Separate repos (`eyewall-pipeline`, `eyewall-poller`) from this one — not started this session.
