...
### Pending
- [ ] French/English localization (Phase A0 + A1 done) — `i18next`/`react-i18next` infra, `localeConfig.js` (localStorage-backed, modeled on `themeConfig.js`), `localeSync.js` (cross-device sync for signed-in users via `user_preferences.preferred_locale`, modeled on `favoriteTeamSync.js`), Settings-popup Language toggle. Global chrome now translated: `BottomNav.jsx`, `Topbar.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `NotificationBell.jsx`, `AccountSection.jsx`. The rest of the app's ~56 remaining view/component files still need string extraction (Phase A2+, view-by-view), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
