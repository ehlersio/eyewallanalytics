...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, and `PeriodSummary.jsx` are all fully translated. This sub-PR added `PWHLPeriodSummary.jsx`, the PWHL sibling of the prior sub-PR's `PeriodSummary.jsx` — structurally near-identical, so nearly every string reused the `periodSummary.*` keys already shipped and live-verified for the NHL version, rather than minting duplicates. Only two new keys were needed: `periodSummary.xCaption.line1Pwhl`/`.line2Pwhl`, because this file's share-caption text uses en dashes (–) between scores where the NHL file uses plain hyphens (-) — a real, if minor, pre-existing difference between the two files, preserved rather than silently unified (the `faceoffWinPct` lesson from the prior sub-PR applied here too: don't assume near-identical files share byte-identical text without checking). Verification hit a new variant of the recurring live-data-gap problem: this component's trigger (a "📊 Game Summary" button in `PWHLShotMapView.jsx`, gated behind `hasPBP && periodSummaries.length > 0`) never appeared for any of several real completed 2025-26 PWHL games tried — those games apparently don't have pre-generated period-summary rows in this environment's data, and unlike NHL's `PeriodSummary.jsx`, there's no `?mockGame=` dev-mock equivalent for PWHL and no dedicated Cypress spec exists either. Verification relied instead on the heavy key reuse from an already live-verified sibling (95%+ of the strings), a full programmatic audit confirming every `t()` key resolves in both locale files, ESLint, and the full vitest suite (185/185, including `pwhlPeriodSummary.test.js`). The rest of the app's ~24 remaining view/component files still need string extraction (long tail: `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
