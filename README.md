...
### Pending
- [ ] French/English localization (Phase A0 + A1 + five A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js` (localStorage-backed, modeled on `themeConfig.js`), `localeSync.js` (cross-device sync for signed-in users via `user_preferences.preferred_locale`, modeled on `favoriteTeamSync.js`), Settings-popup Language toggle. Global chrome translated (`BottomNav.jsx`, `Topbar.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `NotificationBell.jsx`, `AccountSection.jsx`), plus `LeagueView.jsx` + `PWHLLeagueView.jsx` (Standings/Bracket/Leaders/Power Rankings/Draft), `TeamView.jsx` + `PWHLTeamView.jsx` (Overview/Advanced/Splits/Trends/Cap-Salaries/Picks), `PlayersView.jsx` + `PWHLPlayersView.jsx` (Roster/Stats), and `ShotMapView.jsx` + `PWHLShotMapView.jsx` — score bar, metrics grid, shot quality, boxscore, goalie/on-ice cards, and now the stat drill-down popup (all four drill types) + PWHL's `TeamStatsCard` (a gap left by the prior sub-PR, caught and fixed here). Still untranslated: PP/PK analysis panels, live insights/momentum/advanced panels, and the 5-tap debug panel (left for a follow-up sub-PR; the debug panel deliberately stays English-only — never seen by real users). Shared `league.*`/`team.*`/`players.*`/`shotMapView.*` i18n keys reused across NHL/PWHL file pairs where the English text was byte-identical; spelling/casing/wording variants that differ deliberately (NHL "Defensemen" vs PWHL "Defencemen", NHL "shot quality" vs PWHL "Shot Quality", NHL's "High Danger Shots" vs PWHL's "High Danger") kept as separate keys rather than force-merged. The rest of the app's ~45 remaining view/component files still need string extraction (next: ShotMapView's remaining panels — PP/PK analysis, live insights — then the long tail), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
