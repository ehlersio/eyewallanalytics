...
│   │   ├── IceRink.jsx                 # SVG rink — shots, heat map, team-aware — Tailwind (Phase 6), no .css file; .rink-btn shared style extracted to utils/rinkBtnClasses.js (also used by SeasonChipRow.jsx, PWHLShotMapView.jsx, ShotMapView.jsx). `RinkMarkings` + the `W`/`H`/`CX`/`CY` coordinate constants are named exports (Session 100) — the single source of truth for rink geometry, also consumed by LiveEventRink.jsx so the two rinks render pixel-identical
│   │   ├── LiveEventRink.jsx           # Live-game-only compact rink (Session 100) — plots shots/hits/faceoffs/giveaways/takeaways as dots that fade over real elapsed time since first observed (not a hard count/window cutoff), goals pinned/no decay. Renders IceRink.jsx's own exported `RinkMarkings` (not an approximation) so it's pixel-identical to the season shot map; only the coordinate-transform one-liner (`toSvg`) is a local copy, driven by IceRink's own exported W/H/CX/CY. Normalized via each play's own `homeTeamDefendingSide` so home's net is always on the left, stable across period parity. Dot fill = team color (who), dot stroke ring = event-type color matching EventLog's ticker badges exactly (what) — every dot gets a ring, not just goals. Intermission: tracked events clear and a continuous CSS-only Zamboni resurfacing animation (`@keyframes zamboni-drive`, index.css) runs for the whole break, overlaid with the same ordinal+countdown treatment the score bar already shows. The Zamboni is a hand-built multi-shape SVG (halftone-textured body panels, open driver platform, lug-dot wheel hubs) styled after a reference illustration — no image-generation tool (Pixellab or otherwise) is available in this environment. Turns are a curved position bulge + scaleX mirror-flip at each lane's edge, not a rotate(180deg) spin — a flat side-profile vehicle rotated 180deg reads as upside-down mid-turn; mirroring keeps it upright and just flips which way it's facing. NHL-only for now — PWHL's HockeyTech feed lacks penalty coordinates and has no giveaway/takeaway event type at all, scoped as a separate follow-up
