...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-six A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `App.jsx`, `InfoTip.jsx`, `SeasonTypeToggle.jsx`, `SeasonChipRow.jsx`, `GameChipsRow.jsx`, and `TeamOpponentPicker.jsx` are all fully translated. This sub-PR covers the next 4 shared files: `SeasonOverlayChart.jsx` (the Recharts multi-season overlay used by both popups' Compare tabs and `TeamComparisonPopup.jsx` — default `metricLabel`/`xAxisLabel` prop values couldn't call `t()` directly since default params evaluate before hooks are available, so they became `undefined` defaults resolved via `?? t(...)` inside the component body instead; this is also the fix for a real bug live-verified in an earlier sub-PR's testing but not caught at the time — the Compare tab's chart x-axis showed literal English "Game #" in French, now "Match n°"), `LiveEventRink.jsx` (NHL's Session 100 live in-game rink — card title, per-event-type tooltip labels reused by neither `ShotMapView.jsx`'s differently-styled EventLog badges nor built from them, since the two are separate presentations of the same concept; the intermission overlay's ordinal + "cleaning the ice" message reuses `shotMapView.scoreBar.intermission1st/2nd/3rd/OT` for the ordinal part rather than re-deriving it, and this sub-PR deleted the now-fully-unused `periodOrdinal()` helper it replaced), `PlayerComparisonEntry.jsx` (the "vs Player" → "c. Joueur" entry point on both player popups, reusing `PlayerSearch.jsx`'s existing `search.searching`/`search.noResults` keys byte-for-byte), and `SeasonComparisonPicker.jsx` (the shared N-season selector — first use of i18next's `_one`/`_other` plural-suffix convention for a brand-new key in this migration, for the "Only N teams have data" chip tooltip). Also flagged (not fixed, out of scope for this sub-PR): `ShotMapView.jsx`'s own EventLog ticker has a handful of hardcoded English badge labels (GOAL/SHOT/HIT/etc.) missed by its earlier translation pass — spawned as a separate follow-up task rather than fixed inline, since it was found via grep rather than live-testing the feature this sub-PR was actually shipping. All 30 keys used across the 4 files (including both plural forms) audited to resolve in both locale files; ESLint and the full vitest suite (185/185) clean; `player-comparison.cy.js` (16/16), `players.cy.js` (72/72), `pwhl-shots-live.cy.js` (25/25), and `shot-map.cy.js` (48/48) all green. Live-verified in French: `PlayerComparisonEntry`'s toggle/placeholder/no-results states, `SeasonOverlayChart`'s x-axis label, and `SeasonComparisonPicker`'s "Partielles" badge + correctly-pluralized tooltip ("Seulement 4 équipes ont des données…"). Remaining long tail: `TeamPicker.jsx`, `PlayerComparisonPopup.jsx`, `TeamComparisonPopup.jsx`, and `DraftTab.jsx` (large, `DraftTab.jsx` being the biggest single file left at ~586 lines / 30+ strings); `DevReplayView.jsx`/`PWHLDevReplayView.jsx`/`DevDraftView.jsx` are dev-only routes gated behind `import.meta.env.DEV` and never ship to real users, so they're deliberately out of scope. AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) still needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
