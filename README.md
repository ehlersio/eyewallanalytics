...
### Pending
- [ ] French/English localization (Phase A0 + A1 + ten A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, and `ScheduleView.jsx`+`PWHLScheduleView.jsx` are all fully translated. This sub-PR added `components/GameCard.jsx` (the shared `SeriesCard`/`SortBar`/`GameCard` used by `ScheduleView.jsx`) and `components/CalendarView.jsx`+`PWHLCalendarView.jsx` (the calendar-grid views). Found and fixed two real bugs along the way: (1) `GameCard.jsx`'s `SortBar` destructured a `_label` prop that `ScheduleView.jsx` actually passed as `label` — the name mismatch silently dropped it, meaning `SortBar`'s "Sort:"/"Newest first"/"Oldest first"/count-summary text stayed hardcoded English even after `ScheduleView.jsx` itself was fully translated in the prior sub-PR; now fixed by translating the strings directly in `SortBar` (the dead prop is removed from both files) — confirmed live, the previously-stuck text now renders correctly in French. (2) `GameCard.jsx` line 181 had a broken (non-template) string literal — `title="${TEAM_CONFIG.abbr} moneyline"` — so `${TEAM_CONFIG.abbr}` was never interpolated and the tooltip showed that literal text verbatim in production; fixed alongside the translation. Also wired both `CalendarView.jsx` and `PWHLCalendarView.jsx`'s month-nav/weekday-header labels through `formatters.js`'s `formatDate()` Intl wrapper — same hardcoded-English-array bug just fixed in `PWHLScheduleView.jsx` last sub-PR, independently duplicated in both calendar files; French now renders real month/weekday names (e.g. "août 2026", "DIM./LUN./MAR."). One known-and-flagged-but-not-fixed gap: `GameCard.jsx`'s `"📍 Lenovo Center"` venue label is hardcoded for every team regardless of which of the 32 NHL teams is selected — a real pre-existing data-modeling bug (no per-team venue field exists in `teamConfig.js`) unrelated to i18n, out of scope for a translation pass. The rest of the app's ~38 remaining view/component files still need string extraction (long tail: `MatchupDetail.jsx` — the large prediction/head-to-head panel `GameCard.jsx` expands into, still English-only — plus TriviaFeed, StatTileGrid, popups, etc.), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
