...
### Pending
- [ ] French/English localization (Phase A0 + A1 + sixteen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, and `MilestonesFeed.jsx` are all fully translated. This sub-PR added `NewsView.jsx`+`PWHLNewsView.jsx` (the container these three tabs live inside — header, relative-time formatting, source filter chips, pagination, error/empty states, and the attribution footer). Reused `nav.news` for the News tab (byte-identical to the bottom-nav label) and `milestonesFeed.header.title`/`triviaFeed.error.tryAgain`/`triviaFeed.error.workerNotConfigured` for the Milestones tab and shared error/retry strings, rather than minting duplicates. `item.title`/`.excerpt` (real published article content, external sources) stay untouched, same treatment as other non-UI-chrome content this migration. Fixed the same hardcoded-date-formatting bug found five times already in this migration (`timeAgo()`'s `'en-US'` fallback, now routed through `formatters.js`) plus a hardcoded-number-formatting one (`item.score.toLocaleString()` on Reddit excerpts, now `formatNumber()`). The two files' footer attribution text was deliberately NOT merged into one key — the actual source lists differ (team blogs/Bleacher Report/The Athletic/Reddit vs. PWHL Official/IIHF) — while the shared toggle-tab/header/pagination/error strings were. Verified live in French on both NHL (relative times "il y a 3 j"/"30 juin", team-name header, footer) and PWHL (🏒 header, pagination "← Préc." / "Suiv. →", 6-page real article set) news feeds, plus Cypress (`news.cy.js` 16/16, `pwhl-news.cy.js` 15/15, both green). The rest of the app's ~29 remaining view/component files still need string extraction (long tail: `ScoutingTab.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`, `PeriodSummary.jsx`+`PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
