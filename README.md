...
### Pending
- [ ] French/English localization (Phase A0 + A1 + fourteen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, and `TriviaFeed.jsx` are all fully translated. This sub-PR added `StatTileGrid.jsx` (the shared stat-tile-grid + percentile-scale mechanism used by both `PlayerPopup.jsx` and `PWHLPlayerPopup.jsx`'s Stats tabs — percentile scope legend, InfoTip section labels, the "insufficient sample" message, and the current/stale season badge). `def.label`/`def.tip`/`def.calc`/`def.why` themselves are props supplied by the not-yet-translated calling files and correctly left alone. Found and fixed a real i18n-correctness bug along the way, the same class as the four prior hardcoded-date bugs but for ordinals: the percentile markers' rank labels (e.g. "72nd") were built with a hand-rolled English-only `ordinalSuffix()`/`ordinal()` pair. Reused verbatim in French this would have shown "72nd" instead of "72e" — French collapses the st/nd/rd/th distinction to a single "e" (with "1er" for one). Replaced with a new `formatOrdinal()` in `formatters.js`, built on `Intl.PluralRules({type:'ordinal'})` plus a small locale-keyed suffix table, so it generalizes rather than hand-coding French's simpler rule as a special case. Verified live in French on both NHL (`Division`/`Conférence`/`Ligue` legend, real ordinal markers like "78e"/"96e", "En date de 2025-26" stale badge, "Pourquoi c'est important" InfoTip section) and PWHL (single-scope markers, "ACTUEL" current badge) player cards, plus Cypress (`pwhl-players.cy.js` 89/89, `player-comparison.cy.js` 16/16, `team.cy.js` 65/65 executed, `pwhl-team.cy.js` 128/128, all green). Noted for whichever future sub-PR tackles `PlayerPopup.jsx`: it has its own separate, byte-identical copy of the stale-season string (`SkaterHeaderPanel`/`GoalieHeaderPanel`'s radar `staleNote`, 2 call sites) that can reuse this sub-PR's `statTileGrid.section.staleTip` key rather than minting a new one. The rest of the app's ~32 remaining view/component files still need string extraction (long tail: `ScoutingTab.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`, `PeriodSummary.jsx`+`PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
