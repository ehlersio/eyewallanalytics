...
- **`LeagueView`'s empty-standings state crashes (pre-existing, found during the Tailwind migration):** when NHL/PWHL standings genuinely have zero entries (true during the current NHL offseason), `StandingsPanel` renders `SeasonNotStartedState`, which calls `seasonLabelFor(currentSeason)` — but `useSport().currentSeason` isn't guaranteed to be a string at that point, throwing `TypeError: season.slice is not a function` with no error boundary above it, blanking the whole app. Reproduces identically on `main` before this migration (confirmed via `git stash`), so it's unrelated to the CSS/className work — not fixed here since it's out of this sub-PR's scope. Verification for the Standings tab relied on Cypress (which stubs standings data, bypassing this) rather than the live browser for this reason.
