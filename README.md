...
- Probable starters in the game preview (2026-09): a "Probable starters" block in `MatchupDetail`'s Prediction tab (both render paths — including the no-standings path that covers preseason and opening night), from the Worker's `/probable-starters` route via `getProbableStarters(gameId)` (backed by `eyewall-pipeline`'s nightly `starting_goalie.py` — a model of each team's own pattern: share of recent starts, who started last game, back-to-backs, rest; the NHL announces no starters ahead of time, and the model picks the starter ~73–75% of the time in its backtest). `components/ProbableStarters.jsx`: one column per team, the selected team first — the most likely starter with his probability and share of the last 10 starts, "Started last game" / "Back-to-back" / "Day-to-day" tags, and the other goalies smaller. Posted once the game is within 2 days (the block says so before then); renders nothing for preseason, playoff, or finished games, which aren't modeled. Plain probabilities, no betting framing. Team labels use `teamConfig.js`'s new `teamTextColor(abbr)` — each team's WCAG AA `displayColor` on dark, `primaryColor` on light (the same rule `applyTeamTheme()` uses for the selected team's `--team-primary`) — not `nhlApi.js`'s `TEAM_COLORS`, which still holds raw brand colors (FLA/WPG `#041e42` are unreadable on the dark card) and is still used for opponent colors in ~10 other places. Covered by `cypress/e2e/probable-starters.cy.js`, which stubs the team schedule (a trimmed real 2026-27 CAR schedule, `cypress/fixtures/schedule-car-2026-27.json`) and `/probable-starters`.
