...
### Pending
- [ ] French/English localization (Phase A0 + A1 + fifteen A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, and `StatTileGrid.jsx` are all fully translated. This sub-PR added `MilestonesFeed.jsx` (the Milestones tab, shared between NHL/PWHL — hat trick/shorthanded-goal/shutout/season/career milestone badges, the team-filter dropdown, detail-line strings, and loading/error/empty states). `item.description` itself is left untouched — it's API-authored text from `milestones.py`/`pwhl_milestones.py`, not client-side UI chrome. Fixed two bugs of the same classes found repeatedly in this migration: `formatGameDate()`'s hardcoded `'en-US'` date formatting (now routed through `formatters.js`) and `career_points.toLocaleString()`'s hardcoded (locale-less) number formatting (now `formatNumber()`). Also caught and fixed a gender-agreement issue before it shipped: the "loading player" / "player info not available" strings, drafted first with the feminine `joueuse` to match PWHL's roster, would have been wrong for this file's NHL path (masculine `joueur`) since the component is genuinely shared between both sports — reworded to the gender-neutral "profil" instead of picking one gender. `TeamFilterDropdown`'s two local `t` loop variables (shadowing `useTranslation()`'s `t`) were renamed to `tm` before wiring in the hook, per this migration's established shadowing check. Verified live in French on PWHL (real milestone data: hat trick/season-milestone/shorthanded-goal badges, locale-aware date "18 avr.", detail lines, opening a milestone's player popup) — NHL has zero milestones this preseason (same live-data gap as prior sub-PRs) so its fetch-error and popup-loading strings were verified via Cypress fixtures instead (`milestones.cy.js` 13/13, including "tapping a card opens the player popup", all green). The rest of the app's ~31 remaining view/component files still need string extraction (long tail: `ScoutingTab.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`, `PeriodSummary.jsx`+`PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`+`PWHLPlayerPopup.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
