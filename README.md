...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty-seven A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, `PWHLPeriodSummary.jsx`, `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `App.jsx`, `InfoTip.jsx`, `SeasonTypeToggle.jsx`, `SeasonChipRow.jsx`, `GameChipsRow.jsx`, `TeamOpponentPicker.jsx`, `SeasonOverlayChart.jsx`, `LiveEventRink.jsx`, `PlayerComparisonEntry.jsx`, and `SeasonComparisonPicker.jsx` are all fully translated. This sub-PR covers `TeamPicker.jsx` — the full-screen onboarding flow shown on first launch and via Settings' "Change team": the league-choice step (heading, settings hint, per-sport description, the NHL/PWHL non-affiliation disclaimer paragraph) and the team-choice step (back button, heading, the PWHL expansion section's label/badge/aria-label). NHL/PWHL division and conference names (Atlantic, Metropolitan, Eastern, etc.) are deliberately left untranslated, matching the established precedent already set by `LeagueView.jsx`'s standings — they're treated as data-driven proper nouns, not UI copy, throughout this app. All 10 keys resolve in both locale files; ESLint and the full vitest suite (185/185) clean; `TeamPicker.cy.js` (10/10) green. Live-verified in French on both the sport-choice and team-choice steps, including confirming the PWHL expansion section currently never renders (all 4 expansion teams have `comingSoon: false` today, per `pwhlConfig.js`), so that section's translated strings are ready but not yet visually reachable. Remaining long tail: `PlayerComparisonPopup.jsx`, `TeamComparisonPopup.jsx`, and `DraftTab.jsx` (large, `DraftTab.jsx` being the biggest single file left at ~586 lines / 30+ strings); `DevReplayView.jsx`/`PWHLDevReplayView.jsx`/`DevDraftView.jsx` are dev-only routes gated behind `import.meta.env.DEV` and never ship to real users, so they're deliberately out of scope. AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) still needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
