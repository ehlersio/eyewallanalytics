...
### Pending
- [ ] French/English localization (Phase A0 + A1 + two A2 sub-PRs done) — `i18next`/`react-i18next` infra, `localeConfig.js` (localStorage-backed, modeled on `themeConfig.js`), `localeSync.js` (cross-device sync for signed-in users via `user_preferences.preferred_locale`, modeled on `favoriteTeamSync.js`), Settings-popup Language toggle. Global chrome translated (`BottomNav.jsx`, `Topbar.jsx`, `AboutPopup.jsx`, `PlayerSearch.jsx`, `NotificationBell.jsx`, `AccountSection.jsx`), plus `LeagueView.jsx` + `PWHLLeagueView.jsx` (Standings/Bracket/Leaders/Power Rankings/Draft) and `TeamView.jsx` + `PWHLTeamView.jsx` (Overview/Advanced/Splits/Trends/Cap-Salaries/Picks) — shared `league.*`/`team.*` i18n keys reused across NHL/PWHL file pairs where the English text was byte-identical; PWHL's own `Playoff Bracket`/`Power Rankings` tab casing deliberately kept distinct from NHL's sentence-case versions to avoid breaking `pwhl-league.cy.js`'s existing assertions. The rest of the app's ~52 remaining view/component files still need string extraction (next A2 sub-PRs — PlayersView, ShotMapView, etc.), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
