...
### Pending
- [ ] French/English localization (Phase A0 + A1 + twenty A2 sub-PRs done, `PlayerPopup.jsx` now underway in sub-parts like `ShotMapView.jsx` was) — `i18next`/`react-i18next` infra, `localeConfig.js`/`localeSync.js`, Settings-popup Language toggle. Global chrome, `LeagueView.jsx`+`PWHLLeagueView.jsx`, `TeamView.jsx`+`PWHLTeamView.jsx`, `PlayersView.jsx`+`PWHLPlayersView.jsx`, `ShotMapView.jsx`+`PWHLShotMapView.jsx`, `GameEvents.jsx`+`PWHLGameEvents.jsx`, `ScheduleView.jsx`+`PWHLScheduleView.jsx`, `GameCard.jsx`+`CalendarView.jsx`+`PWHLCalendarView.jsx`, `MatchupDetail.jsx`+`PWHLGamePreviewPopup.jsx`, `GameStatsPopup.jsx`+`GameStatsComponents.jsx`+`PWHLGameStatsPopup.jsx`+`PWHLBoxScoreTable.jsx`, `TriviaFeed.jsx`, `StatTileGrid.jsx`, `MilestonesFeed.jsx`, `NewsView.jsx`+`PWHLNewsView.jsx`, `PredictionShareCanvas.jsx`+`PWHLPredictionShareCanvas.jsx`+`ShareButtons.jsx`, `ScoutingTab.jsx`, `PeriodSummary.jsx`, and `PWHLPeriodSummary.jsx` are all fully translated. `PlayerPopup.jsx` (1467 lines, the largest remaining file) is being split into sub-PRs by section rather than done in one pass. This sub-PR covered `RankBadge`, `PlayerRadarChart`, `SkaterHeaderPanel`+`GoalieHeaderPanel`, and `PlayerHeatMap` (the header/radar area and the full Heat Map tab) — the Analytics tab, Results-vs-Process, Scouting Blurb, and the main popup body (Stats tab, bio/contract panel, rankings, tab shell) remain for future sub-PRs. Reused `statTileGrid.section.staleTip` for the header radar's whole-season-fallback note — the exact byte-identical string flagged as a reuse candidate back when `StatTileGrid.jsx` was translated. Found and fixed a second instance of the same ordinal-formatting bug already fixed once this migration (`StatTileGrid.jsx`'s percentile markers): `RankBadge`'s division/conference/league rank suffix (`'st'`/`'nd'`/`'rd'`/`'th'`, hardcoded English) would have shown "3rd" in a French UI. Replaced with the same `formatOrdinal()` from `formatters.js`, dropping the `<sup>`-styled suffix split in favor of the single combined string that call already returns (matching how `StatTileGrid.jsx` uses it) — the now-unused `RANK_NUM_SUP_CLASSES` constant was removed rather than left dangling. Verified live in French: the header radar's stale-season note and the skater Heat Map's empty state both confirmed correct; the populated heat map (filter chips, stat labels, zone map) couldn't be reached live since no NHL shot data exists yet this preseason, so `players.cy.js` (72/72 executed) and `league.cy.js` (34/34 executed) were used instead — both fully green after two dev-server crashes under Cypress load turned out to be the session's previously-documented local-sandbox instability, not real failures (confirmed by restarting and rerunning clean). The rest of the app's ~24 remaining view/component files still need string extraction (long tail: the rest of `PlayerPopup.jsx`, `PWHLPlayerPopup.jsx`, and other popups), and the AI-generated narrative content (scouting blurbs, results-vs-process, game summaries, trivia) needs a `locale` column + dual-generation in `eyewall-pipeline` (Phase B, separate repo) before French narratives exist at all.
