...
// See that file's header comment for the ps-canvas-*/light-mode-overrides.css
...
// Duplicated from PeriodSummary.jsx per established per-file convention.
// See that file's header comment for the full reasoning (dead-class list,
// no markers needed, duplicate-definition resolution).
...
// 'en' (empty net) fix -- this is the file where the bug actually manifests
// (PWHL's strengthLabel() returns 'en', NHL's never does). Given the same
// treatment as 'sh'.
    ...
    <div className={PS_SHARE_CANVAS_CLASSES} ref={canvasRef}>
      ...
      {!isGame && (
        <div className={PS_CANVAS_INSIGHTS_CLASSES}>
          ...
          {(carPens > 0 || oppPens > 0) && (
            <div className={PS_CANVAS_INSIGHT_CHIP_CLASSES}>
              {t('periodSummary.penalties.insight', { abbr: carAbbr, carCount: carPens, oppCount: oppPens, oppAbbr })}
            </div>
          )}
        </div>
      )}
            ...
            {summary.threeStars.slice(0, 3).map((s, i) => {
              ...
              return (
                <div key={i} className={PS_CANVAS_STAR_CLASSES}>
                  ...
                  {s.headshot ? (
                    <img src={s.headshot} alt={name} className={PS_CANVAS_STAR_IMG_CLASSES}
                      onError={e => { e.target.style.display = 'none'; }} />
                  ) : (
      ...
      <div className={PS_CANVAS_FOOTER_CLASSES}>
        ...
        <span className={PS_CANVAS_FOOTER_TAG_CLASSES}>#PWHL</span>
      </div>
    </div>
  );
}
