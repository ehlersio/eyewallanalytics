...
import { pwhlLogoUrl, getPWHLTeamConfig } from '../utils/pwhlConfig';
...
// The share card itself lives in PeriodSummaryShareCanvas.jsx (shared with
// PeriodSummary.jsx); ShareCanvas below only supplies PWHL specifics. Its
// strength colors include 'en' (empty net), which only PWHL returns.
...
function ShareCanvas({ summary, carAbbr, oppAbbr, isCarHome, canvasRef, cardNarrative }) {
  ...
  const insights = [
    dominatedBy && { good: dominatedBy === carAbbr, text: `${dominatedBy === carAbbr ? '↑' : '↓'} ${t('periodSummary.possession.dominated', { team: dominatedBy })}` },
  ...
  return (
    <PeriodSummaryShareCanvas
      ...
      carAbbr={carAbbr}
      oppAbbr={oppAbbr}
      carScore={isCarHome ? summary.homeScore : summary.awayScore}
      oppScore={isCarHome ? summary.awayScore : summary.homeScore}
      carColor={getPWHLTeamConfig(carAbbr)?.displayColor}
      oppColor={getPWHLTeamConfig(oppAbbr)?.displayColor}
      ...
      note={t('shareCard.pwhlStatsNote')}
    />
  );
}
...
export default function PWHLPeriodSummary({
  ...
  return (
    <>
      <div className={`${PS_OVERLAY_CLASSES} pwhl-summary-overlay`} onClick={readOnly ? undefined : onDismiss}>
        <div className={PS_CARD_CLASSES} onClick={e => e.stopPropagation()}>
          ...
          <div className={PS_SCORE_BANNER_CLASSES}>
            <div className={PS_TEAM_SCORE_CLASSES}>
              {carLogo && (
                <img src={carLogo} alt={carAbbr} className={PS_TEAM_LOGO_CLASSES}
                  onError={e => { e.target.style.display = 'none'; }} />
              )}
            ...
            <div className={PS_TEAM_SCORE_CLASSES}>
              {oppLogo && (
                <img src={oppLogo} alt={oppAbbr} className={PS_TEAM_LOGO_CLASSES}
                  onError={e => { e.target.style.display = 'none'; }} />
              )}
