 ...
 * ECHLGameEvents.jsx
 *
 * Game event popups (goal, penalty, win, puck drop) for the ECHL live
 * game view — port of AHLGameEvents.jsx's sessionStorage-deduped popup
 * layer (ECHL parity pass, Phase 6 equivalent). Deliberately does NOT
 * port a live-insights panel, same reasoning as AHL's own file: several
 * of PWHLLiveInsights' callouts (faceoff dominance in particular) depend
 * on event types ECHL's PBP doesn't have at all (no faceoff/hit/
 * blocked_shot events exist for this league either).
 *
 * Event shape (from /echl/live/:gameId, before any further normalization):
 *   { eventType, teamId, period, time, timeSeconds, scoredBy, assists,
 *     takenBy, shotType, isPowerPlay, goalieIn, goalieOut }
 * Confirmed live 2026-08-30 against a real completed game (24296) that
 * penalty/goalie_change carry the exact same field names as AHL's/PWHL's.
 */
...
export function ECHLPuckDropPopup({ data, onClose }) {
    ...
    <div className={overlayClasses(false)} onClick={onClose}>
      <div className={PUCK_DROP_POPUP_CLASSES}>
        ...
        <div className={PUCK_DROP_TEXT_CLASSES}>
          ...
          {t('echlGameEvents.puckDrop.line3')}
        </div>
...
export function ECHLGoalPopup({ data, onClose }) {
  const { t } = useTranslation();
  useEffect(() => {
    if (!data) return;
  ...
  const modifiers = [
    data.isPowerPlay    && t('echlGameEvents.goal.modifierPowerPlay'),
    data.isShortHanded  && t('echlGameEvents.goal.modifierShortHanded'),
    data.isEmptyNet     && t('echlGameEvents.goal.modifierEmptyNet'),
    data.isPenaltyShot  && t('echlGameEvents.goal.modifierPenaltyShot'),
  ].filter(Boolean);
...
export function ECHLPenaltyPopup({ data, onClose }) {
  const { t } = useTranslation();
  useEffect(() => {
    if (!data) return;
    ...
    <div className={overlayClasses(false)} onClick={onClose}>
      <div className={PENALTY_POPUP_CLASSES}>
        <div className={PENALTY_WORDS_CLASSES}>
          <span className={PENALTY_WORD_SPAN_CLASSES}>{t('echlGameEvents.penalty.word1')}</span>
          <span className={PENALTY_WORD_SPAN_CLASSES}>{t('echlGameEvents.penalty.word2')}</span>
        </div>
...
export function ECHLWinPopup({ data, onClose }) {
  const { t } = useTranslation();
  useEffect(() => {
    if (!data) return;
    const timer = setTimeout(onClose, 12000);
...
// confirmed for ECHL as AHL/PWHL (see this file's header comment):
...
const PENALTY_PREFIX_MAP = {
  'maj': 'echlGameEvents.penalty.severity.major',
  'mis': 'echlGameEvents.penalty.severity.misconduct',
  'gm':  'echlGameEvents.penalty.severity.gameMisconduct',
};
...
// ── Period label helper ───────────────────────────────────────
// No isPlayoff branching, unlike PWHL's version -- ECHL has no
// confirmed regular-season shootout period the way PWHL does; penaltyshot
// is a confirmed distinct PBP event type instead of a period-5 shootout
// round (see echl.js's live normalizer comment), so every period past OT
// is just OT2/OT3/etc regardless of season type.
...
// ── useECHLGameEvents hook ────────────────────────────────────
/**
 * Watches liveData.events for new events and fires popups. Direct port
 * of useAHLGameEvents -- see that hook's own comments for the
 * sessionStorage-key/ref-vs-state race rationale, unchanged here.
 *
 * @param {object}  liveData     - from /echl/live/:gameId (raw, before normalization)
...
export function useECHLGameEvents(liveData, isLive, teamId, teamAbbr, isPlayoff = false) {
  ...
  const shownGoals = useRef(new Set(
    gameId ? JSON.parse(sessionStorage.getItem(`echl_goals_${gameId}`) || '[]') : []
  ));
  const shownPenalties = useRef(new Set(
    gameId ? JSON.parse(sessionStorage.getItem(`echl_penalties_${gameId}`) || '[]') : []
  ));
  ...
  useEffect(() => {
    ...
    lastEventIdx.current = gameId
      ? parseInt(sessionStorage.getItem(`echl_lastEvent_${gameId}`) || '-1', 10)
      : -1;
  ...
  useEffect(() => {
    ...
    lastEventIdx.current = eventsLength - 1;
    if (gameId) sessionStorage.setItem(`echl_lastEvent_${gameId}`, String(lastEventIdx.current));
    ...
    for (const ev of newEvents) {
      ...
      if (ev.eventType === 'goal' && ev.teamId === teamId) {
        ...
        if (!shownGoals.current.has(goalSig)) {
          ...
          if (gameId) sessionStorage.setItem(`echl_goals_${gameId}`, JSON.stringify([...shownGoals.current]));
  ...
  useEffect(() => {
    ...
    const sessionKey = `echl_puckdrop_${gameId}`;
  ...
  useEffect(() => {
    ...
    const sessionKey = `echl_win_${gameId}`;
    if (gameId && sessionStorage.getItem(sessionKey)) return;
