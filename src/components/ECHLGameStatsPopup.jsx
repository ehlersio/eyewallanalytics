...
//
// ECHL box-score popup, triggered from a completed game's card in
...
//     -- echl_skater_game_box has no columns for them (confirmed live:
//     always 0 in ECHL's feed, never ingested). Only shots-on-goal and
//     penalty minutes are real here.
//   - No "View Shot Map" CTA -- ECHLShotMapView.jsx is season-aggregate
//     only (no per-game drill-down state exists for ECHL).
...
import ECHLBoxScoreTable from './ECHLBoxScoreTable';
...
export default function ECHLGameStatsPopup({ game, teamId, abbr, color, onClose }) {
  ...
  useEffect(() => {
    capture('echl_game_stats_opened', { gameId: game?.game_id, opponent: oppAbbr });
  }, []);
  ...
  // faceoff% here, unlike PWHL's: echl_skater_game_box has no columns for
          ...
          {!boxLoading && (skaters.length > 0 || goalies.length > 0) && (
            <div className="pgs-section mt-4.5">
              <div className="pgs-skater-toggle flex gap-2 mb-2.5">
                ...
                >
                  <TeamLogo abbr={abbr} sport="echl" size={14} color={color} />
                ...
                >
                  <TeamLogo abbr={oppAbbr} sport="echl" size={14} color={oppColor} />
