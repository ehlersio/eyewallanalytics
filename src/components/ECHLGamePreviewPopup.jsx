...
//
// ECHL pre-game preview popup, triggered from an upcoming game's card in
// ECHLScheduleView.jsx -- port of AHLGamePreviewPopup.jsx, sourcing from:
//   - /echl/preview?gameId=    -- season series, all-time head-to-head,
//     streaks, team-scoped leading scorers, special teams
//   - /echl/prediction?gameId= -- win probability, expected score, AI
//     narrative. No corsiForPct anywhere -- echl_team_seasons has no
//     shot-attempts data source, same as AHL -- so this popup never
//     renders a shot-attempt-share row at all.
//
// ECHL's raw gameCenterPreview payload has the IDENTICAL shape to AHL's,
// confirmed live 2026-08-30 against a real game (game_id 24296) -- not
// PWHL's shape, since ECHL is the same HockeyTech/LeagueStat vendor
// generation as AHL:
...
//
// No prediction-share/export section -- out of this phase's scope.

import React, { useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import { useFetch } from '../hooks/useFetch';
import { formatDate as formatDateIntl } from '../utils/formatters';
import { fetchECHLPreview, fetchECHLPrediction } from '../utils/echlApi';
import { getECHLTeamById } from '../utils/echlConfig';
import TeamLogo from './TeamLogo';
import { capture } from '../utils/analytics';
import { SKELETON_CLASSES } from '../utils/skeletonClasses';
import { saveECHLPrediction, getECHLPredictionStats } from '../utils/echlPredictionStore';
...
export default function ECHLGamePreviewPopup({ game, teamId, abbr, color, onClose }) {
  ...
  useEffect(() => {
    capture('echl_game_preview_opened', { gameId: game?.game_id, opponent: oppAbbr });
  }, []);
  ...
  const predStats = getECHLPredictionStats();
        ...
        <div className="pgp-header p-4 border-b-[0.5px] border-b-[color:var(--border)] relative">
          <div className="pgp-header-inner flex items-center justify-between gap-3">
            <div className={PGP_TEAM_COL_CLASSES}>
              <TeamLogo abbr={abbr} sport="echl" size={36} color={color} />
            ...
            <div className={`${PGP_TEAM_COL_CLASSES} right`}>
              <TeamLogo abbr={oppAbbr} sport="echl" size={36} color={oppColor} />
