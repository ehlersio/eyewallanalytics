...
// Player detail popup for ECHL -- port of AHLPlayerPopup.jsx (ECHL parity
// pass, Phase 2 equivalent of the AHL/PWHL parity plan), scoped to
// Stats + Heat Map tabs only. Deliberately drops, all real data walls
// rather than scope choices, same reasoning as AHL's own popup:
//   - Percentile radar header / percentile-highlighted tiles -- no
//     echl_percentiles.py-equivalent pipeline computation exists.
//   - Scout tab (AI narrative) and Compare tab (season-over-season) --
//     out of this phase's scope, same as AHL's.
//   - "vs Player" comparison entry (PlayerComparisonEntry) -- that
//     component hardcodes an nhl/pwhl branch throughout; needs its own
//     ECHL branch + search-index work, not this phase.
//   - Goalie heat map -- confirmed 2026-08-30 that ECHL's PBP goal events
//     also carry goalie_id: null (echl_shot_events.py's own comment,
//     same structural gap as AHL's feed). Shown as an honest "not
//     available" state instead of a heat map that would silently
//     under-count goals.
//
...
import { fetchECHLPlayerShots, fetchECHLPlayerLanding, fetchECHLPlayerCareer } from '../utils/echlApi';
...
import { SKATER_STATS, GOALIE_STATS, posLabel, groupStats } from '../utils/echlPlayerStats';

// Reuses PlayerPopup.jsx/AHLPlayerPopup.jsx's Tailwind class constants
// verbatim -- same popup-owned-helper convention (duplicate per file
// rather than cross-import between the NHL/PWHL/AHL/ECHL component trees).
...
// Goalie decision letter -- ECHL's echl_game_log has no ot/shootout
// boolean columns either (same as AHL), so gameByGame rows (from
// /echl/player/career) can't distinguish an OT/SO loss from a regulation
// one -- only W/L are derivable here.
function echlGoalieDecision(g) {
  if (g.win) return 'W';
  if (g.loss || g.ot_loss || g.shootout_loss) return 'L';
  return null;
}
function echlDecisionColor(decision) {
  return decision === 'W' ? 'var(--green)' : decision === 'L' ? 'var(--red-bright)' : 'var(--amber)';
}

// ── Heat Map (skaters only) ──────────────────────────────────────────────
function ECHLHeatMap({ playerId, season, isGoalie, teamId }) {
  const { t } = useTranslation();
  const [filter, setFilter] = useState('all');

  const { data: shotData, loading } = useFetch(
    () => !isGoalie && playerId ? fetchECHLPlayerShots(playerId, season) : Promise.resolve(null),
  ...
  if (isGoalie) {
    return (
      <div className={PP_HEATMAP_EMPTY_CLASSES}>
        ...
        <div>{t('echlPlayerPopup.heatMap.goalieUnavailable')}</div>
        <div className={PP_HEATMAP_SUB_CLASSES}>{t('echlPlayerPopup.heatMap.goalieUnavailableSub')}</div>
      </div>
    );
  }
  ...
  const tAbbr = getECHLTeamById(teamId)?.abbr || null;
  const tColor = getECHLTeamById(teamId)?.displayColor || 'var(--team-primary)';
...
// ── Main popup ────────────────────────────────────────────────
export default function ECHLPlayerPopup({ player: initial, seasonLabel, season = ECHL_CURRENT_SEASON, onClose }) {
  const { t } = useTranslation();
  ...
  const { data: landing, loading: statsLoading } = useFetch(
    () => playerId ? fetchECHLPlayerLanding(playerId, season) : Promise.resolve(null),
    [playerId, season]
  );
  ...
  const { data: career } = useFetch(
    () => playerId ? fetchECHLPlayerCareer(playerId) : Promise.resolve(null),
    [playerId]
  );
    ...
    <div className="popup-backdrop" onClick={onClose}>
      <div className={PLAYER_POPUP_CLASSES} onClick={e => e.stopPropagation()}>
        ...
        {/* ── Header — always the plain identity layout (no percentile-driven
            radar reflow the way PWHL's/NHL's headers get, since ECHL has no
        ...
        {/* ── Player Spotlight — draft + bio bullets, from /echl/player/career
              ...
              <>
                {career?.recentGames?.length > 0 && (
                  <div>
                    <div className={PP_FORM_LABEL_CLASSES}>
                      ...
                      <InfoTip text={isGoalie ? t('echlPlayerPopup.recentForm.legendGoalie') : t('playerPopup.recentForm.legendSkater')} position="above" />
                    </div>
                    <div className={PP_FORM_STRIP_CLASSES}>
                      {career.recentGames.map((g, i) => {
                        const decision = isGoalie ? echlGoalieDecision(g) : null;
                        const main = isGoalie
                          ? (decision || '—')
                          : `${g.goals ?? 0}-${g.assists ?? 0}-${g.points ?? 0}`;
                        const color = isGoalie
                          ? echlDecisionColor(decision)
                ...
                {currentGroups.length > 0
                  ? <TileStatSection
                      ...
                      // "2026 Kelly Cup Playoffs") -- rendered as-is, not
                      // run through playerPopup.sections.seasonRegularPwhl's
                      // "{{season}} Regular Season" template like PWHL's
                      // popup does. That template silently produces
                      // "2026 Kelly Cup Playoffs Regular Season" whenever
                      // the label passed in is itself a playoffs season,
                      // which is ECHL's actual live state for most of its
                      // long off-season, same as AHL's (see
                      // echlConfig.js's ECHL_REGULAR_SEASON_MAP comment).
                      label={seasonLabel}
        ...
        {/* ── Heat map tab ── */}
        {ppTab === 'heatmap' && (
          <ECHLHeatMap playerId={p.player_id} season={season} isGoalie={isGoalie} teamId={p.team_id} />
        )}
