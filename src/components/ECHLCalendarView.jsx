...
// Port of AHLCalendarView.jsx -- monthly grid of ECHL games. Completed
...
// variant -- echl_game_log has no ot/shootout boolean columns either
// (same as AHL), so every non-win is just rendered as a plain loss, same
// simplification already applied everywhere else in the ECHL vertical.
...
export function ECHLCalendarView({ games, calMonth, setCalMonth, onGamePopup, teamId }) {
...
function ECHLCalCell({ day, game, isToday, teamId, onGamePopup }) {
  const { t } = useTranslation();
  if (!game) {
    return (
  ...
  const oppColor     = getECHLTeamById(oppId)?.displayColor || 'var(--text-muted)';
  ...
  return (
    ...
    >
      ...
      <div className="cal-cell-body flex items-center gap-1 flex-nowrap min-w-0">
        <TeamLogo abbr={oppAbbr} sport="echl" size={20} color={oppColor} />
