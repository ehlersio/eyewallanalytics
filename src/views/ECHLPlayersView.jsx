...
//
// Player detail popup added (ECHL parity pass, Phase 2 equivalent) --
// roster cards and stat rows are now clickable, mirroring
// AHLPlayersView.
        ...
        <>
          ...
          {!loading && data && roster.length > 0 && (
            <>
              ...
              <RosterSection title={t('echlPlayersView.defencemen')} players={roster.filter(p => ['D','LD','RD'].includes(p.position))} onSelect={setSelected} />
        ...
        <>
          ...
          {statType === 'skaters' && (
            <SortableTable rows={skaters} cols={ECHL_SKATER_COLS} defaultSort="points" loading={loading}
              emptyMsg={t('echlPlayersView.emptySkaterStats', { season: seasonLabel })} onRowClick={setSelected} />
          )}
          {statType === 'goalies' && (
            <SortableTable rows={goalies} cols={ECHL_GOALIE_COLS} defaultSort="wins" loading={loading}
              emptyMsg={t('echlPlayersView.emptyGoalieStats', { season: seasonLabel })} onRowClick={setSelected} />
          )}
        </>
      )}
