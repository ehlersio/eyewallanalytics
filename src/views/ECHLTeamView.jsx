...
// Mirrors AHLTeamView.jsx, scoped to the tabs buildable from data ECHL
// already has: Overview / Stats / Splits / Trends. Same real, confirmed
// data walls as AHL (not a scope choice):
...
import {
  fetchECHLStandings, fetchECHLPlayers, fetchECHLSchedule,
  ECHL_TEAM_CONFIG, ECHL_TEAM_ID,
} from '../utils/echlApi';
...
export default function ECHLTeamView() {
  ...
  // of ECHL's off-season too (see ECHL_REGULAR_SEASON_MAP's comment in
  // echlConfig.js). Same recurring AHL/PWHL gotcha, resolved the same way.
  const regSeasonId = isECHLPlayoffSeason(currentSeason)
    ? (ECHL_REGULAR_SEASON_MAP[currentSeason] ?? currentSeason)
    : currentSeason;
  const poSeasonId = isECHLPlayoffSeason(currentSeason)
    ? currentSeason
    : ECHL_PLAYOFF_SEASON_MAP[currentSeason];

  const { data: standings, loading: sLoad } = useFetch(() => fetchECHLStandings(regSeasonId), [regSeasonId]);
  const { data: players,   loading: pLoad } = useFetch(
    () => teamId ? fetchECHLPlayers(teamId, regSeasonId) : Promise.resolve(null), [teamId, regSeasonId]
  );
  const { data: schedule,  loading: scLoad  } = useFetch(
    () => teamId ? fetchECHLSchedule(teamId, regSeasonId) : Promise.resolve(null), [teamId, regSeasonId]
  );
  const { data: poSchedule, loading: poScLoad } = useFetch(
    () => (teamId && poSeasonId) ? fetchECHLSchedule(teamId, poSeasonId) : Promise.resolve(null), [teamId, poSeasonId]
  );
    ...
    return (
      <div className={PAGE_CLASSES}>
        <div className="card" style={{ textAlign: 'center', padding: 32 }}>
          <p style={{ color: 'var(--text-dim)' }}>{t('echlTeamView.noTeamSelected')}</p>
        </div>
      </div>
    );
  }
  ...
  const seasonLabel = ECHL_SEASONS.find(s => s.id === regSeasonId)?.label || `Season ${regSeasonId}`;
    ...
    <>
      <div className={RECORDS_ROW_CLASSES}>
        <div className={`card ${RECORD_BLOCK_CLASSES}`}>
          <div className={RECORD_BLOCK_LABEL_CLASSES} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <TeamLogo abbr={abbr} sport="echl" size={14} color={color} /> {t('team.regularSeason')}
          </div>
          ...
          {teamRow && (
            <div className={RECORD_META_CLASSES}>
              <span>{t('echlTeamView.overview.gfLabel', { val: teamRow.goals_for??'—' })}</span>
              <span className={RECORD_META_SEP_CLASSES}>·</span>
              <span>{t('echlTeamView.overview.gaLabel', { val: teamRow.goals_against??'—' })}</span>
              {gd != null && (
                <span className={streakChipClasses(gd >= 0)}>
                  {t('echlTeamView.overview.diffSuffix', { sign: gd >= 0 ? '+' : '', gd })}
                </span>
              )}
            </div>
          )}
      ...
      {last5.length > 0 && (
        <div className="card" style={{ marginTop: 10 }}>
          <div className="sec-label" style={{ marginBottom: 8 }}>{t('echlTeamView.overview.last5Title')}</div>
      ...
      {topScorers.length > 0 && (
        <div className="card" style={{ marginTop: 10 }}>
          <div className="sec-label" style={{ marginBottom: 8 }}>{t('echlTeamView.overview.pointsLeadersTitle', { abbr })}</div>
          {topScorers.map((p, i) => (
            <div key={p.player_id ?? i} className={ADV_STAT_ROW_CLASSES}>
              <span className={ADV_STAT_LABEL_CLASSES}>
                {p.player_name || t('echlTeamView.overview.playerFallbackName', { id: p.player_id })}
              ...
              <span className={ADV_STAT_RIGHT_CLASSES}>
                <span className={advStatValClasses()} style={{ color }}>
                  {p.points ?? '—'} {t('echlTeamView.overview.ptsSuffix')}
                </span>
      ...
      {starter && (
        <div className="card" style={{ marginTop: 10 }}>
          <div className="sec-label" style={{ marginBottom: 8 }}>{t('echlTeamView.overview.startingGoalieTitle', { abbr })}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>
                {starter.player_name || t('echlTeamView.overview.playerFallbackName', { id: starter.player_id })}
              </div>
          ...
          {starter.shutouts > 0 && (
            <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text-muted)', paddingLeft: 60 }}>
              {t('echlTeamView.overview.shutoutNote', { count: starter.shutouts })}
            </div>
          )}
        </div>
      )}
    ...
    <>
      {teamTotals && (
        <div className={METRICS_GRID_4_CLASSES} style={{ marginTop: 10 }}>
          <MetCard label={t('echlTeamView.stats.metGoals')}   value={teamTotals.goals}   sub={t('echlTeamView.stats.subTeamTotal')} />
          <MetCard label={t('echlTeamView.stats.metAssists')} value={teamTotals.assists} sub={t('echlTeamView.stats.subTeamTotal')} />
          <MetCard label={t('echlTeamView.stats.metPoints')}  value={teamTotals.points}  sub={t('echlTeamView.stats.subTeamTotal')} color="green" />
          <MetCard label={t('echlTeamView.stats.metPpg')}     value={teamTotals.ppg}     sub={t('echlTeamView.stats.subPowerPlay')} />
        </div>
      )}

      <div className={TABS_WRAP_CLASSES} style={{ marginTop: 8 }}>
        <button className={tabClasses(view === 'skaters')} onClick={() => setView('skaters')}>{t('echlTeamView.stats.viewSkaters')}</button>
        <button className={tabClasses(view === 'goalies')} onClick={() => setView('goalies')}>{t('echlTeamView.stats.viewGoalies')}</button>
      </div>
      ...
      {!loading && view === 'skaters' && skaters.map((p, i) => (
        <div key={p.player_id ?? i} className={ADV_STAT_ROW_CLASSES}>
          <span className={ADV_STAT_LABEL_CLASSES}>
            {p.player_name || t('echlTeamView.stats.playerFallbackShort', { id: p.player_id })}
            {p.position && <span className={ADV_STAT_NOTE_CLASSES}> {p.position}</span>}
          </span>
          <span className={ADV_STAT_RIGHT_CLASSES}>
            <span className={advStatValClasses()} style={{ color }}>
      ...
      {!loading && view === 'goalies' && goalies.map((g, i) => (
        <div key={g.player_id ?? i} className={ADV_STAT_ROW_CLASSES}>
          <span className={ADV_STAT_LABEL_CLASSES}>
            {g.player_name || t('echlTeamView.stats.playerFallbackShort', { id: g.player_id })}
          </span>
          <span className={ADV_STAT_RIGHT_CLASSES}>
            <span className={advStatValClasses()} style={{ color }}>
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:10, marginTop:10 }}>
      ...
      {!splits ? (
        <div className="card" style={{ textAlign:'center', padding:32, color:'var(--text-dim)' }}>
          {t('echlTeamView.splits.noDataYet', { label: label.toLowerCase() })}
        </div>
      ) : (
        <div className="card">
...
function TrendsTab({ schedule, teamId, loading }) {
  ...
  if (loading) return (
    <div className={`card ${EMPTY_STATE_CLASSES}`} style={{ marginTop: 10 }}>
      ...
      <div className={EMPTY_TITLE_CLASSES}>{t('echlTeamView.trends.loadingTitle')}</div>
    </div>
  );
  ...
  if (!gameLog.length) return (
    <div className={`card ${EMPTY_STATE_CLASSES}`} style={{ marginTop: 10 }}>
      <div className={EMPTY_ICON_CLASSES}>📈</div>
      <div className={EMPTY_TITLE_CLASSES}>{t('echlTeamView.trends.noDataTitle')}</div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 10 }}>
      ...
      <div className="card">
        <div className="sec-label" style={{ marginBottom: 10 }}>{t('echlTeamView.trends.rollingWinPctTitle')}</div>
          ...
          {rollDisp.map((g, i) => (
            <div key={i} className={ROLLING_BAR_WRAP_CLASSES}>
              ...
              <div className={rollingBarClasses(g.w10pct)}
                ...
                title={t('echlTeamView.trends.simpleBarTooltip', { pct: g.w10pct })} />
        ...
        <div className={ROLLING_LEGEND_CLASSES}>
          ...
          <span style={{ color:'var(--text-dim)', marginLeft:'auto', fontSize:9 }}>{t('echlTeamView.trends.legendFootnoteShort')}</span>
        </div>
      </div>
        ...
        <div className={GD_CHART_WRAP_CLASSES}>
          ...
          <div className={GD_BARS_CLASSES}>
            {display.map((g, i) => {
              ...
              return (
                <div key={i} className={GD_BAR_COL_CLASSES} title={t('echlTeamView.trends.simpleDotTooltip', { result: g.result, my: g.my, op: g.op })}>
